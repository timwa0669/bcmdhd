...
#ifdef DHD_43022
static int _dhdsdio_secure_firmware_download(struct dhd_bus *bus);
#endif /* DHD_43022 */
...
#ifdef DHD_43022
#define FIRMWARE_STUB_START_ADDR		0x0 /* 0x29d000 */
...
#ifdef DHD_43022
uint sdio_idleclk_disable;
module_param(sdio_idleclk_disable, uint, 0);
...
#ifdef DHD_43022
static int dhdsdio_dongle_host_pre_nvram_dl_handshake(dhd_bus_t *bus, hs_addrs_t *addr);
static int dhdsdio_dongle_host_post_nvram_dl_handshake(dhd_bus_t *bus, hs_addrs_t *addr);
static void dhdsdio_set_sdbus_clk_width(dhd_bus_t *bus, unsigned int flags);
#endif /* DHD_43022 */
...
static int dhdsdio_dongle_host_handshake_spinwait(dhd_bus_t *bus, volatile void *addr,
	uint32 bitshift, uint32 us, uint32 *buffer);
#else
static int dhdsdio_dongle_host_handshake_spinwait(dhd_bus_t *bus, volatile void *addr,
...
#ifdef DHD_43022
/*
 *	This function will check if device access has to be blocked till
 * 	firmware is validated and the reclaimable area is reclaimed to packet pool by fw.
 * 	There are three steps can be taken here:
 *  	1. If the timeout happens then the BL is hung then reset the device and fail insmod.
 *      2. If the FW validation status recieved from BL is fail then reset the device and fail
 *         insmod and retry FW download again as last attempt.
 *      3. If the FW validation status recieved from BL is PASS then continue the normal flow.
 */
static int
dhdsdio_check_ack_wait_dongle_access(struct dhd_bus *bus, hs_addrs_t *addr)
{
	unsigned int h2d_reg = 0x00000000, loop = 0, val = 0;
	int bcmerror = BCME_OK;

	DHD_INFO(("%s: Enter\n", __FUNCTION__));

	if (!bus && !bus->sdh) {
		DHD_ERROR(("%s: Either bus or bus->sdh pointer is null\n", __FUNCTION__));
		bcmerror = BCME_BADARG;
		goto error;
	}

	/* Test for HS bit to be enabled from bootloader to request to disable baclplane.
	 * Timeout is assumed as bootloader hang. So need to reset device and retry insmod.
	 */
	bcmerror = dhdsdio_dongle_host_handshake_spinwait(bus, addr->d2h,
		D2H_BP_CLK_DISABLE_REQ_SHIFT, D2H_BP_CLK_DISABLE_REQ_TO_MS, NULL);
	if (bcmerror != BCME_OK) {
		DHD_ERROR(("%s: Failed to get the bit D2H_BP_CLK_DISABLE_REQ_SHIFT set\n",
			__FUNCTION__));
		goto error;
	}

	/* Set the hs bit to BL to know that, now it can disable backplane */
	bcmerror = dhdsdio_handshake_msg_reg_read(bus, addr->h2d, &h2d_reg);
	if (bcmerror) {
	      DHD_ERROR(("%s: Failed to read from h2d register\n", __FUNCTION__));
	      goto error;
	}
	setbit(&h2d_reg, H2D_BP_CLK_DISABLE_ACK_SHIFT);
	...
	for (loop = 0; loop < LOOP_TO_CHECK_FOR_BP_ENABLE; loop++) {
		val = bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_INTPEND, &bcmerror);
		if (val & SDIOD_CCCR_INTPEND_INT1) {
			DHD_INFO(("%s[%d]: Backplane enabled.\n",  __FUNCTION__, loop));
			break;
		}
	...
	if (loop == LOOP_TO_CHECK_FOR_BP_ENABLE) {
		...
		DHD_ERROR(("%s: Device hung, return failure.\n", __FUNCTION__));
	}
...
#define KSO_DBG(x)	do { if (dhd_msg_level & DHD_TRACE_VAL) printf x; } while (0)
#else
#define KSO_DBG(x)
#endif // endif
...
dhdsdio_clk_kso_enab(dhd_bus_t *bus, bool on)
{
	 ...
	 */
	if ((!on) && (BCM43012_CHIP(bus->sih->chip) ||
			bus->sih->chip == CYW55500_CHIP_ID ||
			bus->sih->chip == CYW55560_CHIP_ID)) {
		return err;
	}
...
dhdsdio_clk_devsleep_iovar(dhd_bus_t *bus, bool on)
{
	...
	if (on == TRUE) {
		...
		if (bus->idleclock == DHD_IDLE_STOP) {
			dhdsdio_set_sdbus_clk_width(bus, SDIO_SDMODE_1BIT);
		}
#endif /* DHD_43022 */
...
#ifdef DHD_43022
		if (bus->idleclock == DHD_IDLE_STOP) {
			dhdsdio_set_sdbus_clk_width(bus, SDIO_SDMODE_4BIT);
		}
#endif /* DHD_43022 */
	}
...
#ifdef DHD_43022
static void
dhdsdio_set_sdbus_clk_width(dhd_bus_t *bus, unsigned int flags)
{
	int ret = 0;
	u8 ctrl;
	bcmsdh_info_t *bcmsdh = bus->sdh;
	...
	if (flags == SDIO_IDLECLOCK_DIS || flags == SDIO_IDLECLOCK_EN) {
		...
		ctrl = bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_BICTRL, &ret);
		if (ret) {
			DHD_ERROR(("Read failed"));
			return;
		}
		ctrl &= ~BUS_SD_DATA_WIDTH_MASK;
		if (flags == SDIO_SDMODE_4BIT)
			ctrl |= BUS_SD_DATA_WIDTH_4BIT;

		bcmsdh_cfg_write(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_BICTRL, ctrl, &ret);
		host->pm_caps |= flags;
		host->ops->set_ios(host, &host->ios);
	}
}
...
dhdsdio_sdclk(dhd_bus_t *bus, bool on)
{
	...
	if (on) {
		if (bus->idleclock == DHD_IDLE_STOP) {
#ifdef DHD_43022
			dhdsdio_set_sdbus_clk_width(bus, SDIO_IDLECLOCK_DIS);
#endif /* DHD_43022 */
		...
		if (bus->idleclock == DHD_IDLE_STOP) {
#ifdef DHD_43022
			dhdsdio_set_sdbus_clk_width(bus, SDIO_IDLECLOCK_EN);
#endif /* DHD_43022 */
			iovalue = 0;
			err = bcmsdh_iovar_op(bus->sdh, "sd_clock", NULL, 0,
			                      &iovalue, sizeof(iovalue), TRUE);
...
#ifdef DHD_43022
	shaddr = bus->dongle_ram_base + CM3_SOCRAM_WRITE_END_LOCATION - 4;
#else
...
#ifdef DHD_43022
	varaddr = NVRAM_DL_ADDR - varsize - TOKENSIZE;
#else
...
#ifdef DHD_43022
		prhex("varslist:", (uchar *)vbuffer, varsize + TOKENSIZE);
#endif /* DHD_43022 */
...
#ifdef DHD_43022
	/* Registers use for Handshake
	 * ToSbMailboxData (Offset 0x48)
	 * ToHostMailboxData (Offset 0x4C)
	 */
	addr->h2d = (uint32 *)&bus->regs->tosbmailboxdata;
	addr->d2h = (uint32 *)&bus->regs->tohostmailboxdata;
#else
#ifndef HS_IN_TCM
...
#ifdef DHD_43022
	bcmerror = dhd_bus_readwrite_bp_addr(bus->dhd, (uint)(uintptr)addr->h2d, sizeof(uint32),
		(uint *)&h2d_reg, FALSE);
#else
...
#ifdef DHD_43022
	bcmerror = dhdsdio_dongle_host_handshake_spinwait(bus, addr->d2h,
			D2H_READY_SHIFT, D2H_READY_TIMEOUT_MS, NULL);
#else
	bcmerror = dhdsdio_dongle_host_handshake_spinwait(bus, addr->d2h,
...
#ifdef DHD_43022
static int
dhdsdio_dongle_host_pre_nvram_dl_handshake(dhd_bus_t *bus, hs_addrs_t *addr)
{
	int bcmerror = BCME_OK;
	int h2d_reg = 0x00000000;

	/* Set DL_NVRAM_START indication to BL that Host shall start NVRAM download */
	setbit(&h2d_reg, H2D_DL_NVRAM_START_SHIFT);
	bcmerror = dhd_bus_readwrite_bp_addr(bus->dhd, (uint)(uintptr)addr->h2d, sizeof(uint32),
		(uint *)&h2d_reg, FALSE);
	if (bcmerror) {
		goto err;
...
static int
dhdsdio_dongle_host_post_nvram_dl_handshake(dhd_bus_t *bus, hs_addrs_t *addr)
{
	int bcmerror = BCME_OK;
	int h2d_reg = 0x00000000;

	/* Reset the NVRAM download start */
	clrbit(&h2d_reg, H2D_DL_NVRAM_START_SHIFT);
	...
	bcmerror = dhdsdio_dongle_host_handshake_spinwait(bus, addr->d2h,
		D2H_MV_NVRAM_DONE_SHIFT, D2H_MV_NVRAM_DONE_TIMEOUT_MS, NULL);
...
dhdsdio_dongle_host_post_handshake(dhd_bus_t *bus, hs_addrs_t *addr)
{
...
#ifdef DHD_43022
	bcmerror = dhdsdio_check_ack_wait_dongle_access(bus, addr);
#else
	bcmerror = dhdsdio_dongle_host_handshake_spinwait(bus, addr->d2h,
	...
	if (bcmerror) {
#ifdef DHD_43022
		DHD_ERROR(("%s(): Return failure as device not accessible\n", __FUNCTION__));
		goto err;
#else
		/* Host notification to bootloader to get reset on error */
		dhdsdio_handshake_msg_reg_read(bus, addr->h2d, &h2d_reg);
...
#ifdef DHD_43022
	bcmerror = dhdsdio_dongle_host_handshake_spinwait(bus, addr->d2h,
			D2H_VALDN_DONE_SHIFT, D2H_VALDN_DONE_TIMEOUT_MS, &d2h_reg);
#else
...
#ifdef DHD_43022
	bcmerror = dhd_bus_readwrite_bp_addr(bus->dhd, (uint)(uintptr)addr,
			sizeof(uint32), (uint *)buffer, FALSE);
#else
#ifndef HS_IN_TCM
...
#ifdef DHD_43022
	bcmerror = dhd_bus_readwrite_bp_addr(bus->dhd, (uint)(uintptr)addr,
			sizeof(uint32), (uint *)buffer, TRUE);
#else
#ifndef HS_IN_TCM
	BCM_REFERENCE(bcmerror);
...
dhd_bus_init(dhd_pub_t *dhdp, bool enforce_mutex)
{
	...
	if ((ret < 0) &&
		...
		(CHIPID(bus->sih->chip) == BCM43439_CHIP_ID) ||
		(CHIPID(bus->sih->chip) == BCM4345_CHIP_ID) ||
		(CHIPID(bus->sih->chip) == BCM4373_CHIP_ID) ||
...
dhdsdio_hostmail(dhd_bus_t *bus, uint32 *hmbd)
{
	...
	if (retries <= retry_limit && hmb_data)
                     ...
                     uint16 devid)
{
...
#ifdef DHD_43022
	if (bus->sih->chip == CYW43022_CHIP_ID) {
		bus->secureboot = !bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
				SBSDIO_FUNC1_SECURE_MODE, &err);
		DHD_ERROR(("%s: This is %ssecured chip..!! \n", __FUNCTION__,
				(bus->secureboot)? "" : "non-"));
	}
#endif /* DHD_43022 */
	...
	if (!DHD_NOPMU(bus)) {
		...
		if (!si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
#ifdef DHD_43022
			if ((((uint16)bus->sih->chip) == CYW43022_CHIP_ID) &&
				(bus->secureboot == TRUE)) {
				bus->orig_ramsize = CM3_43022C1_RAM_SZ;
			}
#else
...
#ifdef DHD_43022
	if (BCM43012_CHIP(bus->sih->chip) && !!sdio_idleclk_disable) {
	...
	if (bus->sih) {
		...
		if (KSO_ENAB(bus) && (dongle_isolation == FALSE)) {
			...
			if (bus->chipidpresent) {
				...
				if (bus->secureboot) {
					cardctl = bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_0,
							SDIOD_CCCR_IOABORT, &bcmerror);
					cardctl |= SDIOD_CCCR_IOABORT_RES;
					if (!bcmerror) {
						bcmsdh_cfg_write(bus->sdh, SDIO_FUNC_0,
								SDIOD_CCCR_IOABORT, cardctl,
								&bcmerror);
					}
				}
#else
...
#ifdef DHD_43022
	if (bus->secureboot) {
		/* 43022, Reading the trx header and writing at 512KB area */
		trx_len = dhd_os_get_image_block((char *)memptr, sizeof(struct trx_header), image);
		if (trx_len < 0) {
			DHD_ERROR(("%s: dhd_os_get_image_block failed (%d)\n",
				__FUNCTION__, trx_len));
			bcmerror = BCME_ERROR;
			goto err;
		}
		if (trx_len >= sizeof(struct trx_header)) {
			trx_hdr = (struct trx_header *)memptr;
			if (trx_hdr->magic == TRX_MAGIC) {
				/* Write a TRX header */
				bcmerror = dhdsdio_membytes(bus, TRUE, TRX_HDR_START_ADDR,
						memptr, sizeof(struct trx_header));
				if (bcmerror) {
					DHD_ERROR(("%s: error %d on writing %d "
								"membytes at 0x%08x\n",
								__FUNCTION__, bcmerror,
								memblock_size, offset));
					goto err;
				}
			}
		}
...
#ifdef DHD_43022
		/* Add start of RAM address to the address given by user */
		offset += FIRMWARE_STUB_START_ADDR;
#endif /* DHD_43022 */
...
dhdsdio_ucode_base(struct dhd_bus *bus)
{
	...
	case CYW43022_CHIP_ID:
	case CYW43023_CHIP_ID:
...
#ifdef DHD_43022
static int
_dhdsdio_secure_firmware_download(struct dhd_bus *bus)
{
	int bcmerror = -1;
	bool dlok = FALSE;	/* download firmware succeeded */
	bool embed = FALSE;	/* download firmware succeeded */
	uint i = 0;
	hs_addrs_t bl_hs_addrs = {NULL, NULL};
	int err = 0;
	...
	/* Bootloader host pre handshake function */
	if ((bcmerror = dhdsdio_dongle_host_pre_nvram_dl_handshake
			(bus, &bl_hs_addrs))) {
		DHD_ERROR(("%s: error %d dongle host pre handshake\n",
			__FUNCTION__, bcmerror));
		goto err;
	}
	/* Chip - 43022 : Download NVRAM before Firmware */
	...
	/* Write vars */
	if ((bcmerror = dhdsdio_write_vars(bus))) {
		DHD_ERROR(("%s: could not write the vars to RAM\n", __FUNCTION__));
		goto err;
	}

	/* NVRAM DOWNLOAD done notification to bootloader */
	if ((bcmerror = dhdsdio_dongle_host_post_nvram_dl_handshake
			(bus, &bl_hs_addrs))) {
		DHD_ERROR(("%s: error %d dongle host post nvram DL handshake\n",
			__FUNCTION__, bcmerror));
		goto err;
	}

	/* FW download pre handshake */
	if ((bcmerror = dhdsdio_dongle_host_pre_handshake
			(bus, &bl_hs_addrs))) {
		DHD_ERROR(("%s: error %d dongle host pre handshake\n",
			__FUNCTION__, bcmerror));
	...
	/* FW download done notification to bootloader */
	if ((bcmerror = dhdsdio_dongle_host_post_handshake(bus,
			&bl_hs_addrs))) {
		DHD_ERROR(("%s: error %d dongle host post handshake\n",
			__FUNCTION__, bcmerror));
	...
	if ((bcmerror = dhdsdio_dongle_host_chk_validation(bus,
			&bl_hs_addrs))) {
		DHD_ERROR(("%s: error %d dongle signature validation\n",
			__FUNCTION__, bcmerror));
		goto err;
	}
...
#ifdef DHD_43022
	/* Secure mode download flow */
	if (bus->secureboot && bus->sih->chip == CYW43022_CHIP_ID) {
		if (_dhdsdio_secure_firmware_download(bus)) {
			DHD_ERROR(("%s:%d bootloader image file download failed\n", __FUNCTION__,
				__LINE__));
			 goto err;
		} else {
			return BCME_OK;
