...
static
#endif /* WL_DHD_XR */
s32 wl_cfg80211_set_wiphy_params(struct wiphy *wiphy, u32 changed);
#ifdef WLAIBSS_MCHAN
...
static
#endif /* WL_DHD_XR */
int wl_cfg80211_update_connect_params(struct wiphy *wiphy, struct net_device *dev,
...
static
#endif /* WL_DHD_XR */
s32 wl_cfg80211_set_pmksa(struct wiphy *wiphy, struct net_device *dev,
	struct cfg80211_pmksa *pmksa);

#ifndef WL_DHD_XR
static
#endif /* WL_DHD_XR */
s32 wl_cfg80211_del_pmksa(struct wiphy *wiphy, struct net_device *dev,
	struct cfg80211_pmksa *pmksa);
#ifndef WL_DHD_XR
static
#endif /* WL_DHD_XR */
...
static
#endif /* WL_DHD_XR */
s32 wl_cfg80211_set_rekey_data(struct wiphy *wiphy, struct net_device *dev,
...
static
#endif /* WL_DHD_XR */
int wl_cfg80211_set_pmk(struct wiphy *wiphy, struct net_device *dev,
        const struct cfg80211_pmk_conf *conf);
#ifndef WL_DHD_XR
static
#endif /* WL_DHD_XR */
int wl_cfg80211_del_pmk(struct wiphy *wiphy, struct net_device *dev,
        const u8 *aa);
...
static
#endif /* WL_DHD_XR */
int wl_cfg80211_set_cqm_rssi_config(struct wiphy *wiphy, struct net_device *dev,
...
static
#endif /* WL_DHD_XR */
int wl_cfg80211_update_owe_info(struct wiphy *wiphy, struct net_device *dev,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)) || \
	defined(DHD_ANDROID_CFG80211_BACKPORT_V2)
#define WL_CFG80211_ADD_KEY_XR(src_pub, dest_pub, wiphy, dev, link_id, key_idx, pairwise, mac_addr, params) \
		wl_cfg80211_add_key_xr(src_pub, dest_pub, wiphy, dev, link_id, key_idx, pairwise, mac_addr, params)
#define WL_CFG80211_CONFIG_DEFAULT_KEY_XR(src_pub, dest_pub, wiphy, dev, link_id, key_idx, unicast, multicast) \
		 wl_cfg80211_config_default_key_xr(src_pub, dest_pub, wiphy, dev, link_id, key_idx, unicast, multicast)
#define WL_CFG80211_DEL_KEY_XR(src_pub, dest_pub, wiphy, dev, link_id, key_idx, pairwise, mac_addr) \
		 wl_cfg80211_del_key_xr(src_pub, dest_pub, wiphy, dev, link_id, key_idx, pairwise, mac_addr)
#define WL_CFG80211_GET_KEY_XR(src_pub, dest_pub, wiphy, dev, link_id, key_idx, pairwise, mac_addr, cookie, callback) \
		 wl_cfg80211_get_key_xr(src_pub, dest_pub, wiphy, dev, link_id, key_idx, pairwise, mac_addr, cookie, callback)
#else
#define WL_CFG80211_ADD_KEY_XR(src_pub, dest_pub, wiphy, dev, link_id, key_idx, pairwise, mac_addr, params) \
		wl_cfg80211_add_key_xr(src_pub, dest_pub, wiphy, dev, -1, key_idx, pairwise, mac_addr, params)
#define WL_CFG80211_CONFIG_DEFAULT_KEY_XR(src_pub, dest_pub, wiphy, dev, link_id, key_idx, unicast, multicast) \
		wl_cfg80211_config_default_key_xr(src_pub, dest_pub, wiphy, dev, -1, key_idx, unicast, multicast)
#define WL_CFG80211_DEL_KEY_XR(src_pub, dest_pub, wiphy, dev, link_id, key_idx, pairwise, mac_addr) \
		wl_cfg80211_del_key_xr(src_pub, dest_pub, wiphy, dev, -1, key_idx, pairwise, mac_addr)
#define WL_CFG80211_GET_KEY_XR(src_pub, dest_pub, wiphy, dev, link_id, key_idx, pairwise, mac_addr, cookie, callback) \
		wl_cfg80211_get_key_xr(src_pub, dest_pub, wiphy, dev, -1, key_idx, pairwise, mac_addr, cookie, callback)
#endif // endif
#endif /* WL_DHD_XR */
...
static void wl_xr_slv_event_handler(struct work_struct *work_data);
#else
...
{
#ifdef WL_DHD_XR_CLIENT
        struct net_device *ndev = DHD_XR_GET_SLAVE_NDEV(cfg);
#else
...
#endif /* WL_NAN */
		{
			wl_release_vif_macaddr(cfg, (u8 *)wdev->netdev->dev_addr, wl_iftype);
		}
...
#ifdef WL_DHD_XR_CLIENT
	if (DHD_XR_GET_SLAVE_WDEV(cfg) != NULL &&
		DHD_XR_GET_SLAVE_NDEV(cfg) != NULL) {
...
#ifdef WL_DHD_XR_MASTER
	dhd_pub_t *dhdxr = NULL;
	if (DHD_XR_GET_SLAVE_WDEV(cfg) != NULL &&
		DHD_XR_GET_SLAVE_NDEV(cfg) != NULL) {
		dhdxr = (dhd_pub_t *)dhd_get_pub(DHD_XR_GET_SLAVE_NDEV(cfg));
		/* This callback should get executed for SLAVE driver as well */
		if (dhdxr && DHD_GET_XR_ROLE(dhdxr) == XR_SLAVE) {
			err = wl_cfg80211_set_wiphy_params_xr(cfg->pub, dhdxr, wiphy, changed);
			/* in-case slave fails to set wiphy params, return */
			if (err != BCME_OK) {
				WL_ERR(("error (%d)\n", err));
				return err;
			}
	...
	struct cfg80211_connect_params *sme, u32 changed)
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev
			&& cfg->xr_sta_active_wdev)) {
			dhd_pub_t *xr_sta_pub = (dhd_pub_t *) dhd_get_pub(cfg->xr_sta_active_wdev->netdev);

			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_update_connect_params_xr(cfg->pub,
						cfg->xr_slave_dhd_pub, wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), sme, changed);
			}
		}
	}

	if (dhdp && DHD_GET_XR_ROLE(dhdp) == XR_SLAVE) {
		return wl_cfg80211_update_connect_params_xr(cfg->pub, dhdp, wiphy, dev, sme, changed);
	}
#endif /* WL_DHD_XR_MASTER */
	...
	struct cfg80211_external_auth_params *params)
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev)
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_external_auth_xr(cfg->pub, cfg->xr_slave_dhd_pub,
					wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), params);
			}
		}
	}
...
#ifdef WL_DHD_XR_CLIENT
	err = cfg80211_external_auth_request(xr_sta_ndev, &params, GFP_ATOMIC);
#else
	err = cfg80211_external_auth_request(ndev, &params, GFP_ATOMIC);
#endif /* WL_DHD_XR_CLIENT */
	if (err) {
		WL_ERR(("EXT_AUTH_REQ to supplicant failed : %d\n", err));
	}
	return err;
}
	...
	struct cfg80211_connect_params *sme)
{
...
#ifdef WL_DHD_XR_MASTER
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE && cfg->xr_sta_enabled) {
		u32 xr_sta_chan = 0;
		int xr_sta_band = -1;
		...
		if (sme->channel) {
			xr_sta_chan = ieee80211_frequency_to_channel(sme->channel->center_freq);
			xr_sta_band = sme->channel->band;
		}
		else if (sme->channel_hint) {
			xr_sta_chan = ieee80211_frequency_to_channel(sme->channel_hint->center_freq);
			xr_sta_band = sme->channel_hint->band;
		}
		else {
			WL_ERR(("sme->channel is null\n"));
			return -EINVAL;
		}
		/* sta connection on master or slave based on
		*  AP's created in order to ensure that sta does'nt
		*  disrupt AP operation by triggering VSDB mode on chip.
		*  Eg. 2g AP on Master and 5g AP on Slave.
		*  so sta connection for 5g should be created on the slave.
		*  since there already exists a 5g AP on slave.
		*/
		if (dev->ieee80211_ptr == cfg->wdev && (xr_sta_band != -1)) {
			xr_sta_pub = xr_sta_get_conn_pub(cfg, xr_sta_band);

			WL_DBG(("xr_sta_chan:%d, xr_sta_band:%d\n", xr_sta_chan, xr_sta_band));
			/* Find if any AP's have been brought up already
			*  If AP exists on requested band isse connect command
			*  on the driver to which that AP belongs
			*/
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				/* Issue connect command on Slave*/
				WL_DBG(("Issue connect command on Slave\n"));
				cfg->xr_sta_conn_evt_wdev = DHD_XR_GET_SLAVE_WDEV(cfg);
				xr_sta_sm_evt_handler(cfg, XR_STA_EVT_CONNECT);
				return wl_cfg80211_connect_xr(cfg->pub, cfg->xr_slave_dhd_pub,
						wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), sme);
			} else if (xr_sta_pub) {
				/* Issue connect command on Master*/
				WL_DBG(("Issue connect command on Master\n"));
				cfg->xr_sta_conn_evt_wdev = cfg->wdev;
				xr_sta_sm_evt_handler(cfg, XR_STA_EVT_CONNECT);
			} else {
				/* Fall back to 2g on Master and 5g/6g on slave
				*  as there is no AP brought up or AP exists but none
				*  are on the requested band
				*/
				WL_DBG(("Fall back to 2g on Master and 5/6g on Slave\n"));
				if (!CHANNEL_IS_2G(xr_sta_chan)) {
					cfg->xr_sta_conn_evt_wdev = DHD_XR_GET_SLAVE_WDEV(cfg);
					xr_sta_sm_evt_handler(cfg, XR_STA_EVT_CONNECT);
					return wl_cfg80211_connect_xr(cfg->pub, cfg->xr_slave_dhd_pub,
							wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), sme);
	...
	if (dhdp && DHD_GET_XR_ROLE(dhdp) == XR_SLAVE) {
		return wl_cfg80211_connect_xr(cfg->pub, dhdp, wiphy, dev, sme);
	}
#endif /* WL_DHD_XR_MASTER */
	...
	if (p2p_is_on(cfg) && (dev != bcmcfg_to_prmry_ndev(cfg))) {
		/* we only allow to connect using virtual interface in case of P2P */
			...
			wl_cfg80211_set_mgmt_vndr_ies(cfg, ndev_to_cfgdev(dev), bssidx,
				VNDR_IE_ASSOCREQ_FLAG, sme->ie, sme->ie_len);
#ifdef WL_DHD_XR_CLIENT
	} else if (dev == DHD_XR_GET_SLAVE_NDEV(cfg)) {
#else
...
static void wl_cfg80211_disconnect_state_sync(struct bcm_cfg80211 *cfg, struct net_device *dev)
{
	...
	uint8 wait_cnt;
#ifdef WL_DHD_XR_CLIENT
	struct net_device *xr_sta_ndev = xr_sta_get_ndev_for_cfg_event(cfg, dev);
#endif /* WL_DHD_XR_CLIENT */
...
#ifdef WL_DHD_XR_CLIENT
	wdev = xr_sta_ndev->ieee80211_ptr;
#else
	...
	u16 reason_code)
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				err =  wl_cfg80211_disconnect_xr(cfg->pub, cfg->xr_slave_dhd_pub,
					wiphy, cfg->xr_sta_active_wdev->netdev, reason_code);
				xr_sta_sm_evt_handler(cfg, XR_STA_EVT_DISCONNECT);
				return err;
			}
	...
	if (dhdp && DHD_GET_XR_ROLE(dhdp) == XR_SLAVE) {
		return wl_cfg80211_disconnect_xr(cfg->pub, dhdp, wiphy, dev, reason_code);
	}
#endif /* WL_DHD_XR_MASTER */
...
#ifdef WL_DHD_XR
			/* sta disconnection complete need to change active slave interface */
			if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
				&& cfg->xr_sta_enabled) {
				xr_sta_sm_evt_handler(cfg, XR_STA_EVT_DISCONNECT);
			}
...
#if defined(WL_CFG80211_P2P_DEV_IF)
#ifdef WL_DHD_XR
	dhd_pub_t *dhdxr = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
	struct net_device *ndev = dhd_linux_get_primary_netdev(dhdxr);
#else
	struct net_device *ndev = bcmcfg_to_prmry_ndev(cfg);
#endif /*WL_DHD_XR*/
...
#ifdef WL_DHD_XR_MASTER
	if (dhdxr && DHD_GET_XR_ROLE(dhdxr) == XR_SLAVE) {
		return wl_cfg80211_set_tx_power_xr(cfg->pub, dhdxr, wiphy, wdev, type, mbm);
	}
#endif/* WL_DHD_XR_MASTER */
#else
#ifdef WL_DHD_XR
	/* Get master & slave ndev */
	struct net_device *ndevs = DHD_XR_GET_SLAVE_NDEV(cfg);
	dhd_pub_t *dhdxrs = dhd_get_pub(ndevs);
#ifdef WL_DHD_XR_MASTER
	struct net_device *ndev = bcmcfg_to_prmry_ndev(cfg);
#endif /* WL_DHD_XR_MASTER */
#else
...
#ifdef WL_DHD_XR_MASTER
	if (dhdxrs && DHD_GET_XR_ROLE(dhdxrs) == XR_SLAVE) {
		/* Set tx power for slave */
		err = wl_cfg80211_set_tx_power_xr(cfg->pub, dhdxrs, wiphy, type, dbm);
		if (unlikely(err)) {
			WL_ERR(("error (%d)\n", err));
			return err;
		}
...
#else
#ifdef WL_DHD_XR_CLIENT
	err = wl_set_tx_power(ndevs, type, dbm);
#else
	err = wl_set_tx_power(ndev, type, dbm);
#endif /* WL_DHD_XR_CLIENT */
#endif /* WL_CFG80211_P2P_DEV_IF */
...
#endif /* WL_CFG80211_P2P_DEV_IF */
{
	...
	if (pub && DHD_GET_XR_ROLE(pub) == XR_SLAVE) {
		...
		if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
			&& cfg->xr_sta_enabled) {
			if((ndev == cfg->wdev->netdev && cfg->xr_sta_active_wdev)) {
				dhd_pub_t *xr_sta_pub = (dhd_pub_t *) dhd_get_pub(cfg->xr_sta_active_wdev->netdev);

				if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
#if defined(WL_CFG80211_P2P_DEV_IF)
					return wl_cfg80211_get_tx_power_xr(cfg->pub, cfg->xr_slave_dhd_pub,
						wiphy, DHD_XR_GET_SLAVE_WDEV(cfg), dbm);
#else
...
#ifdef WL_DHD_XR_MASTER
	if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
		return WL_CFG80211_CONFIG_DEFAULT_KEY_XR(cfg->pub, dhd, wiphy, dev, link_id, key_idx, unicast, multicast);
	}
#endif /* WL_DHD_XR_MASTER */
	...
	struct key_params *params)
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return WL_CFG80211_ADD_KEY_XR(cfg->pub, cfg->xr_slave_dhd_pub,
					wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), link_id, key_idx, pairwise, mac_addr, params);
			}
		}
	}
	if (dhdp && DHD_GET_XR_ROLE(dhdp) == XR_SLAVE) {
		return WL_CFG80211_ADD_KEY_XR(cfg->pub, dhdp, wiphy, dev, link_id, key_idx, pairwise, mac_addr, params);
	}
#endif /* WL_DHD_XR_MASTER */
	...
	u8 key_idx, bool pairwise, const u8 *mac_addr)
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE) {
		if (dev == cfg->wdev->netdev) {
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return WL_CFG80211_DEL_KEY_XR(cfg->pub, cfg->xr_slave_dhd_pub,
					wiphy, DHD_XR_GET_SLAVE_NDEV(cfg),link_id_xr, key_idx, pairwise, mac_addr);
			}
		}
	}

	if (dhdp && DHD_GET_XR_ROLE(dhdp) == XR_SLAVE) {
		return WL_CFG80211_DEL_KEY_XR(cfg->pub, dhdp, wiphy, dev, link_id_xr, key_idx, pairwise, mac_addr);
	}
#endif /* WL_DHD_XR_MASTER */
	...
	void (*callback) (void *cookie, struct key_params * params))
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev)
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return WL_CFG80211_GET_KEY_XR(cfg->pub,
					cfg->xr_slave_dhd_pub, wiphy, DHD_XR_GET_SLAVE_NDEV(cfg),
						link_id_xr, key_idx, pairwise, mac_addr, cookie, callback);
			}
		}
	}

	if (dhdp && DHD_GET_XR_ROLE(dhdp) == XR_SLAVE) {
		return WL_CFG80211_GET_KEY_XR(cfg->pub, dhdp, wiphy, dev, link_id_xr, key_idx, pairwise, mac_addr, cookie, callback);
...
#endif // endif
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev)
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_get_station_xr(cfg->pub, cfg->xr_slave_dhd_pub, wiphy,
					 DHD_XR_GET_SLAVE_NDEV(cfg), mac, sinfo);
			}
		}
	}
...
#ifdef WL_DHD_XR
	dhd_pub_t *dhdp = (dhd_pub_t *)dhd_get_pub(dev);
	struct net_device *primary_dev =dhd_linux_get_primary_netdev(dhdp);
#else
...
#ifdef WL_DHD_XR
		pmkid_list_v2_t *pmkid_v2_list = (pmkid_list_v2_t *)MALLOCZ(dhdp->osh, v2_list_size);
#else
...
#ifdef WL_DHD_XR
		MFREE(dhdp->osh, pmkid_v2_list, v2_list_size);
#else
...
#ifdef WL_DHD_XR
		pmkid_list_v1_t *pmkid_v1_list = (pmkid_list_v1_t *)MALLOCZ(dhdp->osh, v1_list_size);
#else
...
#ifdef WL_DHD_XR
		MFREE(dhdp->osh, pmkid_v1_list, v1_list_size);
#else
	...
	struct cfg80211_pmksa *pmksa)
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev)
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_set_pmksa_xr(cfg->pub, cfg->xr_slave_dhd_pub,
					wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), pmksa);
			}
		}
	}

	if (dhdp && DHD_GET_XR_ROLE(dhdp) == XR_SLAVE) {
		return wl_cfg80211_set_pmksa_xr(cfg->pub, dhdp, wiphy, dev, pmksa);
	}
#endif /* WL_DHD_XR_MASTER */
	...
	struct cfg80211_pmksa *pmksa)
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev)
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_del_pmksa_xr(cfg->pub, cfg->xr_slave_dhd_pub,
					wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), pmksa);
			}
		}
	}
	...
	if (dhdp && DHD_GET_XR_ROLE(dhdp) == XR_SLAVE) {
		return wl_cfg80211_del_pmksa_xr(cfg->pub, dhdp, wiphy, dev, pmksa);
	}
#endif /* WL_DHD_XR_MASTER */
...
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0) */
{
	...
	s32 chan_nr;
#endif // endif
#if defined(WL_DHD_XR) && defined(WL_DHD_XR_MASTER)
	dhd_pub_t *dhdp = NULL;
#endif /* WL_DHD_XR */
	WL_DBG(("Enter \n"));
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev)
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0))
				return wl_cfg80211_mgmt_tx_xr(cfg->pub,
					cfg->xr_slave_dhd_pub, wiphy,
						DHD_XR_GET_SLAVE_CFGDEV(cfg), params, cookie);
#else
				return wl_cfg80211_mgmt_tx_xr(cfg->pub,
					cfg->xr_slave_dhd_pub, wiphy,
						DHD_XR_GET_SLAVE_CFGDEV(cfg), channel, offchan,
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(3, 7, 0))
 ...
 */
			...
			*cookie = (u64)mf_params->data;
			...
			/* Abort on going scan if any before triggering auth frame*/
			wl_cfg80211_cancel_scan(cfg);
			...
			if (!test_bit(MGMT_TX_NOACK, &netinfo->mgmt_txstatus) &&
				((timeout > 0) || test_bit(MGMT_TX_ACK, &netinfo->mgmt_txstatus))) {
				...
				if (test_bit(MGMT_TX_NOACK, &netinfo->mgmt_txstatus)) {
					s32 ret = 0;
					/* if no ack, need to reset msch request handle by issuing mf_abort
					 * so that next ext Auth can be sent out without waiting msch slot end.
					 */
					ret = wldev_iovar_setint_bsscfg(dev, "mf_abort",
						netinfo->mgmt_txid, bssidx);
					if (ret < 0) {
						WL_ERR(("mf_abort error %d\n", ret));
					}
				}
			}
txstatus:
	...
	bool privacy)
{
	...
	if (dev_role == NL80211_IFTYPE_P2P_GO && (ies->wpa2_ie)) {
		...
		if (bss->rsn_ie) {
			MFREE(oshp, bss->rsn_ie, bss->rsn_ie[1]
				+ WPA_RSN_IE_TAG_FIXED_LEN);
			bss->rsn_ie = NULL;
		}
		if (bss->wpa_ie) {
			MFREE(oshp, bss->wpa_ie, bss->wpa_ie[1]
				+ WPA_RSN_IE_TAG_FIXED_LEN);
			bss->wpa_ie = NULL;
		}
		if (bss->wps_ie) {
			MFREE(oshp, bss->wps_ie, bss->wps_ie[1] + 2);
			bss->wps_ie = NULL;
		}
		if (bss->fils_ind_ie) {
			MFREE(oshp, bss->fils_ind_ie, bss->fils_ind_ie[1]
				+ FILS_INDICATION_IE_TAG_FIXED_LEN);
		}
		if (ies->wpa_ie != NULL) {
			/* WPAIE */
			bss->rsn_ie = NULL;
			bss->wpa_ie = MALLOCZ(oshp,
					ies->wpa_ie->length
					+ WPA_RSN_IE_TAG_FIXED_LEN);
			if (bss->wpa_ie) {
		...
		if (ies->wps_ie) {
			bss->wps_ie = MALLOCZ(oshp, ies->wps_ie_len);
			if (bss->wps_ie) {
...
#endif // endif
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev)
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_change_station_xr(cfg->pub, cfg->xr_slave_dhd_pub,
						wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), mac, params);
			}
		}
	}
...
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 0)) || defined(DHD_ANDROID_CFG80211_BACKPORT_V1) */
	{
...
#ifdef WL_DHD_XR
	survey = (struct wl_dump_survey *)MALLOCZ(dhd->osh,
	...
	const wl_event_msg_t *e, void *data)
{
	...
	u32 reason = ntoh32(e->reason);
	u32 status = ntoh32(e->status);
#if defined(BIGDATA_SOFTAP) || defined(WL_DHD_XR)
#ifdef WL_DHD_XR
	dhd_pub_t *dhdp = (dhd_pub_t *)dhd_get_pub(ndev) ;
	struct net_device *primary_dev = dhd_linux_get_primary_netdev(dhdp);
#else
	dhd_pub_t *dhdp;
#endif /* WL_DHD_XR */
#endif /* BIGDATA_SOFTAP || WL_DHD_XR */
...
static u32
wl_get_chipnum(struct bcm_cfg80211 *cfg,
	struct net_device *ndev)
{
	u32 chipnum;
	wlc_rev_info_t revinfo;
	...
	ret = wldev_ioctl_get(ndev, WLC_GET_REVINFO, &revinfo, sizeof(revinfo));
	if (ret < 0) {
		WL_ERR(("wl_get_chipnum: GET revinfo FAILED. ret:%d\n", ret));
		return false;
	}

	WL_DBG(("wl_get_chipnum: GET_REVINFO device 0x%x, vendor 0x%x, chipnum 0x%x\n",
		dtoh32(revinfo.deviceid), dtoh32(revinfo.vendorid), dtoh32(revinfo.chipnum)));
	...
	const wl_event_msg_t *e, void *data)
{
...
#endif /* DHD_LOSSLESS_ROAMING */
#ifdef WL_DHD_XR
	dhdp = (dhd_pub_t *)dhd_get_pub(ndev);
	primary_dev = dhd_linux_get_primary_netdev(dhdp);
#else
	dhdp = (dhd_pub_t *)(cfg->pub);
#endif /* WL_DHD_XR_CLIENT */
	BCM_REFERENCE(dhdp);
	...
	if (mode == WL_MODE_AP) {
...
#if defined(OEM_ANDROID)
#ifdef WL_DHD_XR
		DHD_DISABLE_RUNTIME_PM((dhd_pub_t *)dhdp);
#else
		...
		if (wl_is_linkup(cfg, e, ndev)) {
			...
			if (!wl_get_drv_status(cfg, DISCONNECTING, ndev)) {
				...
				WL_DBG(("joined in BSS network \"%s\"\n",
					((struct wlc_ssid *)wl_read_prof(cfg, ndev,
						WL_PROF_SSID))->SSID));
				/* WMM WAR: Disable Frameburst for WMM Cert TC 5.2.28
				 * Due to high Tx, Endpoint not able to receive the packets
				 * from BRCM STA as per TC, cause failure at step 6
				*/
				if ((BCM4373_CHIP_ID == wl_get_chipnum(cfg, ndev)) &&
					(ndev->ieee80211_ptr->iftype == NL80211_IFTYPE_STATION)) {

					int  get_frmbrst = 0, ret = 0;
					ret = wldev_ioctl_get(ndev, WLC_GET_FAKEFRAG,
						&get_frmbrst, sizeof(int));
					if (ret < 0) {
						WL_ERR(("GET framebust mode failed ret:%d\n", ret));
					}
					if (get_frmbrst) {
						uint8 OUI_RALINK[] = {0x00, 0x0c, 0x43};
						uint8 SSID[] = "12345678";
						u8 *ralink_bssid = NULL;
						struct wlc_ssid *ralink_ssid = NULL;

						ralink_ssid = (struct wlc_ssid *)wl_read_prof(cfg,
									ndev, WL_PROF_SSID);
						ralink_bssid = wl_read_prof(cfg,
									ndev, WL_PROF_BSSID);
						if (!memcmp(ralink_bssid, OUI_RALINK, 3) &&
							!memcmp(ralink_ssid->SSID, SSID, 8)) {
							WL_INFORM_MEM(("11N: WMM WAR "
									":Disable frameburst\n"));
							ret = wl_cfg80211_set_frameburst(
											cfg, FALSE);
							if (ret < 0) {
								WL_ERR(("Failed to set frameburst"
										" ret:%d\n", ret));
							}
						}
					}
			...
			if (wl_get_drv_status(cfg, CONNECTED, ndev)) {
#ifdef WL_DHD_XR
				wl_flush_fw_log_buffer(primary_dev,
					FW_LOGSET_MASK_ALL);
#else
				wl_flush_fw_log_buffer(bcmcfg_to_prmry_ndev(cfg),
					FW_LOGSET_MASK_ALL);
...
#ifdef RSSI_MONITOR_SUPPORT
#ifdef WL_DHD_XR
				dhd_dev_set_rssi_monitor_cfg(primary_dev,
				    FALSE, 0, 0);
#else
...
#ifdef WL_DHD_XR_CLIENT
				CFG80211_DISCONNECTED(xr_sta_ndev, reason, ie_ptr, ie_len,
					loc_gen, GFP_KERNEL);
#else
			...
			if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE && cfg->xr_sta_enabled) {
				...
				*/
				if (!((event == WLC_E_LINK) &&
					...
					if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
						&& cfg->xr_sta_enabled
						&& cfg->xr_sta_active_wdev == ndev->ieee80211_ptr) {
						xr_sta_sm_evt_handler(cfg, XR_STA_EVT_DISCONNECT);
					}
#endif /* WL_DHD_XR */
...
#ifdef WL_DHD_XR
			((dhd_pub_t *)dhdp)->aifsn_reverse = FALSE;
#else
...
#if defined(OEM_ANDROID)
#ifdef WL_DHD_XR
		DHD_ENABLE_RUNTIME_PM((dhd_pub_t *)dhdp);
#else
...
#ifdef WL_DHD_XR_CLIENT
	struct net_device *xr_sta_ndev = NULL;
	struct wireless_dev *xr_sta_wdev = NULL;
#endif /* WL_DHD_XR_CLIENT */
	...
	if (datalen) {
		...
		if (evt_data->version == ROAM_EXP_EVENT_VERSION) {
			...
			if (ndev) {
				BCM_REFERENCE(wdev);
#ifdef WL_DHD_XR_CLIENT
			/* For XR single STA supp/cfg80211 sees only master primary I/F(wlan0)
			 * In the case where sta is operating on slave driver we receive the event for
			 * wlan1 , but since supp/cfg80211 see only wlan0 we need to update wlan0's wdev */
				xr_sta_ndev = xr_sta_get_ndev_for_cfg_event(cfg, ndev);
				xr_sta_wdev = xr_sta_ndev->ieee80211_ptr;
				wdev = ndev->ieee80211_ptr;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 2)) || \
	defined(DHD_ANDROID_CFG80211_BACKPORT_V1)
				xr_sta_wdev->u.client.ssid_len = min(ssid->SSID_len, (uint32)DOT11_MAX_SSID_LEN);
				memcpy(xr_sta_wdev->u.client.ssid, ssid->SSID, xr_sta_wdev->u.client.ssid_len);
#else
				xr_sta_wdev->ssid_len = min(ssid->SSID_len, (uint32)DOT11_MAX_SSID_LEN);
				memcpy(xr_sta_wdev->ssid, ssid->SSID, xr_sta_wdev->ssid_len);
...
#ifdef WL_DHD_XR_CLIENT
			wl_cfgvendor_send_async_event(wiphy, xr_sta_ndev,
	...
	const wl_event_msg_t *e, void *data)
{
	...
	s32 rssi = 0;
#ifdef WL_DHD_XR_CLIENT
	struct net_device *xr_sta_ndev = xr_sta_get_ndev_for_cfg_event(cfg, wdev->netdev);
#endif /* WL_DHD_XR_CLIENT */
...
#ifdef WL_DHD_XR_CLIENT
	cfg80211_cqm_rssi_notify(xr_sta_ndev,
...
#ifdef WL_DHD_XR_CLIENT
	cfg80211_cqm_beacon_loss_notify(xr_sta_ndev, GFP_KERNEL);
#else
...
#ifdef WL_DHD_XR_CLIENT
	wl_cfgvendor_send_async_event(wiphy, xr_sta_ndev, GOOGLE_ROAM_EVENT_START,
...
#endif // endif
#ifdef WL_DHD_XR_CLIENT
	cfg80211_roamed(xr_sta_ndev, &roam_info, GFP_KERNEL);
#else
	cfg80211_roamed(ndev, &roam_info, GFP_KERNEL);
#endif /* WL_DHD_XR */
#else
#ifdef WL_DHD_XR_CLIENT
	cfg80211_roamed(xr_sta_ndev,
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 39)) || defined(WL_COMPAT_WIRELESS)
		notify_channel,
#endif // endif
		curbssid,
...
#endif	/* WL_BAM */
#ifdef WL_DHD_XR
			/* sta connection complete need to change active slave interface*/
			if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
				&& cfg->xr_sta_enabled
				&& cfg->xr_sta_active_wdev == ndev->ieee80211_ptr) {
				xr_sta_sm_evt_handler(cfg, XR_STA_EVT_CONNECT_DONE);
			}
#endif /* WL_DHD_XR */
	...
	const wl_event_msg_t *e, void *data, bool completed)
{
	...
	dhdp = (dhd_pub_t *)dhd_get_pub(ndev);
	primary_dev = (struct net_device *)dhd_linux_get_primary_netdev(dhdp);
#else
	dhdp = (dhd_pub_t *)(cfg->pub);
#endif /* WL_DHD_XR */
	BCM_REFERENCE(dhdp);
	...
	if (wl_get_drv_status(cfg, CONNECTING, ndev)) {
...
#endif /* WL_FILS_ROAM_OFFLD */
#ifdef WL_DHD_XR_CLIENT
			cfg80211_connect_done(xr_sta_ndev, &resp_params, GFP_KERNEL);
#else
		...
		{
#ifdef WL_DHD_XR_CLIENT
			CFG80211_CONNECT_RESULT(xr_sta_ndev,
				curbssid,
				bss,
				conn_info->req_ie,
				conn_info->req_ie_len,
	...
	const wl_event_msg_t *e, void *data)
{
	...
	wl_flush_fw_log_buffer(ndev, FW_LOGSET_MASK_ALL);
#ifdef WL_DHD_XR_CLIENT
	cfg80211_michael_mic_failure(xr_sta_ndev, (const u8 *)&e->addr, key_type, -1,
		NULL, GFP_KERNEL);
#else
...
#ifdef WL_DHD_XR_CLIENT
	err = wl_genl_send_msg(xr_sta_ndev, event, data, (u16)datalen, 0, 0);
#else
...
#ifdef WL_DHD_XR_CLIENT
		cfg80211_update_owe_info_event(xr_sta_ndev, &owe_info, GFP_KERNEL);
#else
...
#ifdef WL_DHD_XR_CLIENT
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0))
	 cfg80211_rx_mgmt(xr_sta_ndev->ieee80211_ptr, freq, 0,  mgmt_frame, mgmt_frame_len, 0);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 12, 0))
	cfg80211_rx_mgmt(xr_sta_ndev->ieee80211_ptr, freq, 0,  mgmt_frame, mgmt_frame_len, 0, GFP_ATOMIC);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0)) || \
	defined(WL_COMPAT_WIRELESS)
	cfg80211_rx_mgmt(xr_sta_ndev->ieee80211_ptr, freq, 0, mgmt_frame, mgmt_frame_len, GFP_ATOMIC);
#else
	cfg80211_rx_mgmt(xr_sta_ndev->ieee80211_ptr, freq, mgmt_frame, mgmt_frame_len, GFP_ATOMIC);
...
#ifdef WL_DHD_XR_CLIENT
static void wl_init_xr_slv_event_handler(struct bcm_cfg80211 *cfg)
{
	bzero(cfg->xr_slv_evt_handler, sizeof(cfg->xr_slv_evt_handler));

	cfg->xr_slv_evt_handler[WLC_E_SCAN_COMPLETE] = wl_notify_scan_status;
	cfg->xr_slv_evt_handler[WLC_E_AUTH] = wl_notify_connect_status;
	cfg->xr_slv_evt_handler[WLC_E_ASSOC] = wl_notify_connect_status;
	cfg->xr_slv_evt_handler[WLC_E_LINK] = wl_notify_connect_status;
	cfg->xr_slv_evt_handler[WLC_E_DEAUTH_IND] = wl_notify_connect_status;
	cfg->xr_slv_evt_handler[WLC_E_DEAUTH] = wl_notify_connect_status;
	cfg->xr_slv_evt_handler[WLC_E_DISASSOC_IND] = wl_notify_connect_status;
	cfg->xr_slv_evt_handler[WLC_E_ASSOC_IND] = wl_notify_connect_status;
	cfg->xr_slv_evt_handler[WLC_E_REASSOC_IND] = wl_notify_connect_status;
	cfg->xr_slv_evt_handler[WLC_E_ROAM] = wl_notify_roaming_status;
	cfg->xr_slv_evt_handler[WLC_E_MIC_ERROR] = wl_notify_mic_status;
	cfg->xr_slv_evt_handler[WLC_E_SET_SSID] = wl_notify_connect_status;
	cfg->xr_slv_evt_handler[WLC_E_ACTION_FRAME_RX] = wl_notify_rx_mgmt_frame;
	cfg->xr_slv_evt_handler[WLC_E_PROBREQ_MSG] = wl_notify_rx_mgmt_frame;
	cfg->xr_slv_evt_handler[WLC_E_P2P_PROBREQ_MSG] = wl_notify_rx_mgmt_frame;
	cfg->xr_slv_evt_handler[WLC_E_P2P_DISC_LISTEN_COMPLETE] = wl_cfgp2p_listen_complete;
	cfg->xr_slv_evt_handler[WLC_E_ACTION_FRAME_COMPLETE] = wl_cfgp2p_action_tx_complete;
	cfg->xr_slv_evt_handler[WLC_E_ACTION_FRAME_OFF_CHAN_COMPLETE] = wl_cfgp2p_action_tx_complete;
	cfg->xr_slv_evt_handler[WLC_E_JOIN] = wl_notify_connect_status;
	cfg->xr_slv_evt_handler[WLC_E_START] = wl_notify_connect_status;
	cfg->xr_slv_evt_handler[WLC_E_AUTH_IND] = wl_notify_connect_status;
	cfg->xr_slv_evt_handler[WLC_E_ASSOC_RESP_IE] = wl_notify_connect_status;
#ifdef PNO_SUPPORT
	cfg->xr_slv_evt_handler[WLC_E_PFN_NET_FOUND] = wl_notify_pfn_status;
#endif /* PNO_SUPPORT */
#ifdef GSCAN_SUPPORT
	cfg->xr_slv_evt_handler[WLC_E_PFN_BEST_BATCHING] = wl_notify_gscan_event;
	cfg->xr_slv_evt_handler[WLC_E_PFN_SCAN_COMPLETE] = wl_notify_gscan_event;
	cfg->xr_slv_evt_handler[WLC_E_PFN_GSCAN_FULL_RESULT] = wl_notify_gscan_event;
	cfg->xr_slv_evt_handler[WLC_E_PFN_BSSID_NET_FOUND] = wl_notify_gscan_event;
	cfg->xr_slv_evt_handler[WLC_E_PFN_BSSID_NET_LOST] = wl_notify_gscan_event;
	cfg->xr_slv_evt_handler[WLC_E_PFN_SSID_EXT] = wl_notify_gscan_event;
	cfg->xr_slv_evt_handler[WLC_E_GAS_FRAGMENT_RX] = wl_notify_gscan_event;
	cfg->xr_slv_evt_handler[WLC_E_ROAM_EXP_EVENT] = wl_handle_roam_exp_event;
#endif /* GSCAN_SUPPORT */
#ifdef RSSI_MONITOR_SUPPORT
	cfg->xr_slv_evt_handler[WLC_E_RSSI_LQM] = wl_handle_rssi_monitor_event;
#endif /* RSSI_MONITOR_SUPPORT */
#ifdef WLTDLS
	cfg->xr_slv_evt_handler[WLC_E_TDLS_PEER_EVENT] = wl_tdls_event_handler;
#endif /* WLTDLS */
	cfg->xr_slv_evt_handler[WLC_E_BSSID] = wl_notify_roaming_status;
#ifdef WLAIBSS
	cfg->xr_slv_evt_handler[WLC_E_AIBSS_TXFAIL] = wl_notify_aibss_txfail;
#endif /* WLAIBSS */
#ifdef	WL_RELMCAST
	cfg->xr_slv_evt_handler[WLC_E_RMC_EVENT] = wl_notify_rmc_status;
#endif /* WL_RELMCAST */
#ifdef BT_WIFI_HANDOVER
	cfg->xr_slv_evt_handler[WLC_E_BT_WIFI_HANDOVER_REQ] = wl_notify_bt_wifi_handover_req;
#endif // endif
#ifdef WL_NAN
	cfg->xr_slv_evt_handler[WLC_E_NAN_CRITICAL] = wl_cfgnan_notify_nan_status;
	cfg->xr_slv_evt_handler[WLC_E_NAN_NON_CRITICAL] = wl_cfgnan_notify_nan_status;
#endif /* WL_NAN */
	cfg->xr_slv_evt_handler[WLC_E_CSA_COMPLETE_IND] = wl_csa_complete_ind;
	cfg->xr_slv_evt_handler[WLC_E_AP_STARTED] = wl_ap_start_ind;
#ifdef CUSTOM_EVENT_PM_WAKE
	cfg->xr_slv_evt_handler[WLC_E_EXCESS_PM_WAKE_EVENT] = wl_check_pmstatus;
#endif	/* CUSTOM_EVENT_PM_WAKE */
#if defined(DHD_LOSSLESS_ROAMING) || defined(DBG_PKT_MON)
	cfg->xr_slv_evt_handler[WLC_E_ROAM_PREP] = wl_notify_roam_prep_status;
#endif /* DHD_LOSSLESS_ROAMING || DBG_PKT_MON  */
	cfg->xr_slv_evt_handler[WLC_E_ROAM_START] = wl_notify_roam_start_status;
#ifdef WL_BAM
	cfg->xr_slv_evt_handler[WLC_E_ADPS] = wl_adps_event_handler;
#endif	/* WL_BAM */
	cfg->xr_slv_evt_handler[WLC_E_PSK_SUP] = wl_cfg80211_sup_event_handler;
#ifdef WL_BCNRECV
	cfg->xr_slv_evt_handler[WLC_E_BCNRECV_ABORTED] = wl_bcnrecv_aborted_event_handler;
#endif /* WL_BCNRECV */
#ifdef WL_MBO
	cfg->xr_slv_evt_handler[WLC_E_MBO] = wl_mbo_event_handler;
#endif  /* WL_MBO */
#ifdef WL_CAC_TS
	cfg->xr_slv_evt_handler[WLC_E_ADDTS_IND] = wl_cfg80211_cac_event_handler;
	cfg->xr_slv_evt_handler[WLC_E_DELTS_IND] = wl_cfg80211_cac_event_handler;
#endif /* WL_CAC_TS */
#if defined(WL_MBO) || defined(WL_OCE)
	cfg->xr_slv_evt_handler[WLC_E_PRUNE] = wl_bssid_prune_event_handler;
#endif /* WL_MBO || WL_OCE */
#ifdef RTT_SUPPORT
	cfg->xr_slv_evt_handler[WLC_E_PROXD] = wl_cfg80211_rtt_event_handler;
#endif // endif
#ifdef WL_CHAN_UTIL
	cfg->xr_slv_evt_handler[WLC_E_BSS_LOAD] = wl_cfg80211_bssload_report_event_handler;
#endif /* WL_CHAN_UTIL */
	cfg->xr_slv_evt_handler[WLC_E_IND_DOS_STATUS] = wl_notify_dos_status;
#ifdef WL_SAE
	cfg->xr_slv_evt_handler[WLC_E_EXT_AUTH_REQ] = wl_notify_extauth_req_event;
	cfg->xr_slv_evt_handler[WLC_E_EXT_AUTH_FRAME_RX] = wl_notify_rx_mgmt_frame;
	cfg->xr_slv_evt_handler[WLC_E_MGMT_FRAME_TXSTATUS] = wl_notify_mgmt_frame_tx_complete;
	cfg->xr_slv_evt_handler[WLC_E_MGMT_FRAME_OFF_CHAN_COMPLETE] = wl_notify_mgmt_frame_tx_complete;
#endif /* WL_SAE */
#ifdef ENABLE_HOGSQS
	cfg->xr_slv_evt_handler[WLC_E_LDF_HOGGER] = wl_cfg80211_hogsqs_notify;
#endif /* ENABLE_HOGSQS */

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 11, 0))
	cfg->xr_slv_evt_handler[WLC_E_RSSI] = wl_notify_rssi_change_ind;
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(4, 11, 0) */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0))
	cfg->xr_slv_evt_handler[WLC_E_BCNLOST_MSG] = wl_notify_beacon_loss;
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0) */
#if defined(WL_OWE) && (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 2, 0))
	cfg->xr_slv_evt_handler[WLC_E_EXT_ASSOC_FRAME_RX] = wl_notify_rx_mgmt_frame;
#endif /* WL_OWE && LINUX_VERSION_CODE >= KERNEL_VERSION(5, 2, 0) */
	cfg->xr_slv_evt_handler[WLC_E_ESCAN_RESULT] = wl_escan_handler;
}
...
static s32 wl_create_event_handler(struct bcm_cfg80211 *cfg)
{
	...
	if (!cfg->event_xr_slv_workq) {
		cfg->event_xr_slv_workq = alloc_workqueue("dhd_eventd_xr_slv",
	...
	if (!cfg->event_xr_slv_workq) {
		...
		INIT_WORK(&cfg->event_xr_slv_work, wl_xr_slv_event_handler);
	}
#else
		INIT_WORK(&cfg->event_xr_slv_work, wl_xr_slv_event_handler);
		cfg->event_xr_slv_workq_init = true;
#endif /* OEM_ANDROID */
#else
#ifdef OEM_ANDROID
	/* Allocate workqueue for event */
...
#ifdef WL_DHD_XR_CLIENT
#ifdef OEM_ANDROID
	if (cfg && cfg->event_xr_slv_workq) {
		cancel_work_sync(&cfg->event_xr_slv_work);
		destroy_workqueue(cfg->event_xr_slv_workq);
		cfg->event_xr_slv_workq = NULL;
	}
#else
	if (cfg && cfg->event_xr_slv_workq_init) {
		cancel_work_sync(&cfg->event_xr_slv_work);
		cfg->event_xr_slv_workq_init = false;
	}
#endif /* OEM_ANDROID */
#else
#ifdef OEM_ANDROID
...
s32 wl_xr_slv_init_cfg_event(struct bcm_cfg80211 *cfg)
{
	...
	if (wl_create_event_handler(cfg)) {
		WL_ERR(("%s xr slave cfg event handler init failed\n", __func__));
		return BCME_ERROR;
	}
	wl_init_xr_slv_event_handler(cfg);
	return ret;
	...
	unsigned long state, void *ptr)
{
	...
	if (netif_is_bond_master(dev)) {
		...
		/* Confirm if bond interface matches the bond interface
		*  to which station interface belongs.
		*  netdev_master_upper_dev_get() has to be called only after RTNL_LOCK
		*  has been taken(in the notifier callback the RTNL_LOCK has already been acquired)
		**/
		if (state == NETDEV_UP) {
			if (cfg && cfg->wdev
				&& dev == netdev_master_upper_dev_get(cfg->wdev->netdev)) {
				cfg->xr_sta_bond_ndev = dev;
				cfg->xr_sta_bond_if_cnt++;
				WL_DBG(("%s is added to bond interface %s\n",
					cfg->wdev->netdev->name, dev->name));
			}

			if (cfg && DHD_XR_GET_SLAVE_WDEV(cfg)
				&& dev == netdev_master_upper_dev_get(DHD_XR_GET_SLAVE_NDEV(cfg))) {
				cfg->xr_sta_bond_ndev = dev;
				cfg->xr_sta_bond_if_cnt++;
				WL_DBG(("%s is added to bond interface %s\n",
			...
			if (cfg && cfg->xr_sta_bond_ndev
					&& (cfg->xr_sta_bond_if_cnt == MAX_XR_STA_BOND_IF_NUM)) {
				cfg->xr_sta_enabled = TRUE;
				WL_DBG(("bonding I/F updated dev:%s\n",
					 cfg->xr_sta_bond_ndev->name));
			}
		}
		if (state == NETDEV_DOWN) {
			/* When bond interface  disable xr sta */
			if (cfg && cfg->xr_sta_bond_ndev
				&& (dev == cfg->xr_sta_bond_ndev)
				&& (state == NETDEV_DOWN)) {

				WL_DBG(("NETDEV_DOWN case\n"));
	...
	enum wl_status state, bool set)
{
	...
	if (set) {
		...
		if (_net_info->ndev->ieee80211_ptr->ps == TRUE) {
			pm = PM_OFF;
		...
		if (cfg->wdev) {
			...
			if (ret > 0) {
				...
				if (cfg->p2p_net) {
					...
					memcpy_s((void *)mac_addr, ETH_ALEN, ndev->dev_addr, ETH_ALEN);
					mac_addr[0] |= 0x02;
					ETH_HW_ADDR_SET(cfg->p2p_net, mac_addr);
...
void wl_cfg80211_detach_xr(struct bcm_cfg80211 *cfg, dhd_pub_t *dhd)
{
	...
	/* Clean up for slave event handler */
	wl_destroy_event_handler(cfg);
	wl_flush_eq(cfg);
	wl_dealloc_netinfo_by_wdev(cfg, wdev);
	if (wdev) {
...
static void wl_xr_slv_event_handler(struct work_struct *work_data)
{
	...
	WL_DBG(("Enter \n"));
	BCM_SET_CONTAINER_OF(cfg, work_data, struct bcm_cfg80211, event_xr_slv_work);
	pub = (dhd_pub_t *) dhd_get_pub(DHD_XR_GET_SLAVE_NDEV(cfg));
	cfg->wl_xr_slv_evt_hdlr_entry_time = OSL_LOCALTIME_NS();
	...
	while ((e = wl_deq_event(cfg))) {
		...
		cfg->wl_xr_slv_evt_deq_time = OSL_LOCALTIME_NS();
		if (scan_cmplt_evt) {
		...
		if (!(wdev = wl_get_wdev_by_fw_idx(cfg,
			...
			if (e->etype != WLC_E_IF)
				WL_ERR(("No wdev corresponding to bssidx: 0x%x found!"
					" Ignoring event.\n", e->emsg.bsscfgidx));
		} else if (e->etype < WLC_E_LAST && cfg->xr_slv_evt_handler[e->etype]) {
			...
			} else
			{
				...
				cfg->xr_slv_evt_handler[e->etype](cfg, wdev_to_cfgdev(wdev),
					&e->emsg, e->edata);
				if (scan_cmplt_evt) {
...
fail:
		...
		cfg->wl_xr_slv_evt_hdlr_exit_time = OSL_LOCALTIME_NS();
	}
...
wl_cfg80211_event(struct net_device *ndev, const wl_event_msg_t * e, void *data)
{
...
#ifdef OEM_ANDROID
#ifdef WL_DHD_XR_CLIENT
	if (cfg->event_xr_slv_workq == NULL) {
...
#else
#ifdef WL_DHD_XR_CLIENT
	if (!cfg->event_xr_slv_workq_init) {
		WL_ERR(("Event handler is not created\n"));
		return;
	}
#else
...
#ifdef WL_DHD_XR_CLIENT
#ifdef OEM_ANDROID
		queue_work(cfg->event_xr_slv_workq, &cfg->event_xr_slv_work);
#else
		schedule_work(&cfg->event_xr_slv_work);
#endif /* OEM_ANDROID */
	...
	if ((event_type == WLC_E_ESCAN_RESULT) &&
		...
		WL_INFORM_MEM(("Enqueing escan completion (%d). WQ state:0x%x \n",
			status, work_busy(&cfg->event_xr_slv_work)));
#else
		WL_INFORM_MEM(("Enqueing escan completion (%d). WQ state:0x%x \n",
...
static void wl_init_eq(struct bcm_cfg80211 *cfg)
{
	wl_init_eq_lock(cfg);
#ifdef WL_DHD_XR_CLIENT
	INIT_LIST_HEAD(&cfg->eq_xr_slv_list);
#else
...
#ifdef WL_DHD_XR_CLIENT
	while (!list_empty_careful(&cfg->eq_xr_slv_list)) {
		BCM_SET_LIST_FIRST_ENTRY(e, &cfg->eq_xr_slv_list, struct wl_event_q, eq_list);
		list_del(&e->eq_list);
...
static struct wl_event_q *wl_deq_event(struct bcm_cfg80211 *cfg)
{
	...
	flags = wl_lock_eq(cfg);
#ifdef WL_DHD_XR_CLIENT
	if (likely(!list_empty(&cfg->eq_xr_slv_list))) {
		BCM_SET_LIST_FIRST_ENTRY(e, &cfg->eq_xr_slv_list, struct wl_event_q, eq_list);
		list_del(&e->eq_list);
	}
#else
	...
	const wl_event_msg_t *msg, void *data)
{
	...
	flags = wl_lock_eq(cfg);
#ifdef WL_DHD_XR_CLIENT
	list_add_tail(&e->eq_list, &cfg->eq_xr_slv_list);
#else
...
#endif /* WL_NAN  */
#ifdef WL_DHD_XR
	xr_sta_init(cfg, ndev, XR_STA_MODE_SINGLE);
#endif /* WL_DHD_XR */
	INIT_DELAYED_WORK(&cfg->pm_enable_work, wl_cfg80211_work_handler);
	wl_set_drv_status(cfg, READY, ndev);
	WL_ERR(("ndev->name:%s is READY\n",ndev->name));
...
static s32 __wl_cfg80211_down_xr(struct bcm_cfg80211 *cfg, struct net_device *ndev)
{
	...
	for_each_ndev(cfg, iter, next) {
		...
		if ((iter->ndev->ieee80211_ptr->iftype == NL80211_IFTYPE_STATION ||
			...
			xr_sta_ndev = xr_sta_get_ndev_for_cfg_event(cfg, iter->ndev);
			CFG80211_DISCONNECTED(xr_sta_ndev, 0, NULL, 0, false, GFP_KERNEL);
#ifdef WL_DHD_XR
			/* sta disconnection complete need to change active slave interface */
			if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
				&& cfg->xr_sta_enabled
				&& cfg->xr_sta_active_wdev == iter->ndev->ieee80211_ptr) {
				xr_sta_sm_evt_handler(cfg, XR_STA_EVT_DISCONNECT);
			}
#endif /* WL_DHD_XR */
		...
		if ((iter->ndev->ieee80211_ptr->iftype == NL80211_IFTYPE_STATION) &&
			...
			struct wiphy *wiphy = bcmcfg_to_wiphy(cfg);
			struct wireless_dev *wdev = xr_sta_ndev->ieee80211_ptr;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 2)) || \
	defined(DHD_ANDROID_CFG80211_BACKPORT_V1)
...
get_iface_num(const char *name, struct bcm_cfg80211 *cfg)
{
	int i = 0;
	for (i = 0; i < DHD_NUM_STATIC_IFACES; i++) {
		if (cfg->static_ndev[i] && strcmp(name, cfg->static_ndev[i]->name) == 0) {
			return i;
		}
	}
	return BCME_ERROR;
...
static unsigned long wl_lock_eq(struct bcm_cfg80211 *cfg)
{
	unsigned long flags;
#ifdef WL_DHD_XR_CLIENT
	WL_CFG_EQ_LOCK(&cfg->eq_xr_slv_lock, flags);
#else
...
{
#ifdef WL_DHD_XR_CLIENT
	WL_CFG_EQ_UNLOCK(&cfg->eq_xr_slv_lock, flags);
#else
...
{
#ifdef WL_DHD_XR_CLIENT
	spin_lock_init(&cfg->eq_xr_slv_lock);
#else
	...
	case WLC_E_TDLS_PEER_CONNECTED :
		if (cfg->tdls_mgmt_frame) {
#ifdef WL_DHD_XR_CLIENT
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0))
			cfg80211_rx_mgmt(xr_sta_ndev->ieee80211_ptr, cfg->tdls_mgmt_freq, 0,
			...
			cfg80211_rx_mgmt(xr_sta_ndev->ieee80211_ptr, cfg->tdls_mgmt_freq, 0,
					...
					GFP_ATOMIC);
#else
			cfg80211_rx_mgmt(xr_sta_ndev->ieee80211_ptr, cfg->tdls_mgmt_freq,
...
wl_cfg80211_valid_chanspec_p2p(chanspec_t chanspec)
{
	...
	/* channel 149 to 161 */
	else if ((chanspec >= 0xd095) && (chanspec <= 0xd0a1)) {
		valid = true;
	}
#else
	/* channel 36 to 48 */
	...
	size_t count, loff_t *ppos)
{
	...
	while ((token = strsep(&params, " ")) != NULL) {
		...
		tokens = sscanf(token, "%"BUF_S(SUBLOGLEVEL)"s %u", sublog, &log_on);
		if (colon != NULL)
			*colon = ':';
...
{
#ifdef WL_DHD_XR_CLIENT
	dhd_pub_t *dhd = (dhd_pub_t *) dhd_get_pub(DHD_XR_GET_SLAVE_NDEV(cfg));
#else
	dhd_pub_t *dhd = (dhd_pub_t *)(cfg->pub);
#endif /* WL_DHD_XR_CLIENT */
...
#ifdef WL_6E
		err = wldev_iovar_getint(ndev, "chanspec", &chnsp);
		if (unlikely(err)) {
			WL_ERR((" Chanspec failed %d\n",err));
			return err;
		}
		band = chnsp & WL_CHANSPEC_BAND_MASK;
#endif /* WL_6E */
	...
	struct wiphy *wiphy)
{
	...
	if (CHSPEC_BW(chanspec) == WL_CHANSPEC_BW_80) {
		...
		else if (sb == WL_CHANSPEC_CTL_SB_UU || sb == WL_CHANSPEC_CTL_SB_UL) {
			/* CFG API is taking care of only 10MHz difference */
			chandef->center_freq1 = chandef->center_freq1 - 20;
		}
	}
...
wl_cfg80211_ch_switch_notify(struct net_device *dev, uint16 chanspec, struct wiphy *wiphy)
{
...
#ifdef WL_DHD_XR_CLIENT
	struct net_device *xr_sta_ndev = NULL;
	struct bcm_cfg80211 *xr_slv_cfg = NULL;
#endif /* WL_DHD_XR_CLIENT */
...
#endif // endif
#ifdef WL_DHD_XR_CLIENT
	xr_slv_cfg = wl_get_cfg(dev);
	xr_sta_ndev = xr_sta_get_ndev_for_cfg_event(xr_slv_cfg, dev);
	...
	CFG80211_CH_SWITCH_NOTIFY(xr_sta_ndev, &chandef, 0, 0);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION (3, 5, 0) && (LINUX_VERSION_CODE <= (3, 7, \
	\
	0)))
	freq = chan_info.freq;
	cfg80211_ch_switch_notify(xr_sta_ndev, freq, chan_info.chan_type);
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION (3, 8, 0)) */
		...
		struct cfg80211_gtk_rekey_data *data)
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev)
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_set_rekey_data_xr(cfg->pub, cfg->xr_slave_dhd_pub,
						wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), data);
			}
		}
	}

	if (dhdp && DHD_GET_XR_ROLE(dhdp) == XR_SLAVE) {
		return wl_cfg80211_set_rekey_data_xr(cfg->pub, dhdp, wiphy, dev, data);
	}
#endif /* WL_DHD_XR_MASTER */
	...
	const struct cfg80211_pmk_conf *conf)
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev)
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_set_pmk_xr(cfg->pub, cfg->xr_slave_dhd_pub,
					wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), conf);
			}
		}
	}

	if (dhdp && DHD_GET_XR_ROLE(dhdp) == XR_SLAVE) {
		return wl_cfg80211_set_pmk_xr(cfg->pub, dhdp, wiphy, dev, conf);
	}
#endif /* WL_DHD_XR_MASTER */
	...
	const u8 *aa)
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev)
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_del_pmk_xr(cfg->pub, cfg->xr_slave_dhd_pub,
					wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), aa);
			}
		}
	}

	if (dhdp && DHD_GET_XR_ROLE(dhdp) == XR_SLAVE) {
		return wl_cfg80211_del_pmk_xr(cfg->pub, dhdp, wiphy, dev, aa);
	}
#endif /* WL_DHD_XR_MASTER */
...
wl_cfg80211_get_sta_channel(struct bcm_cfg80211 *cfg)
{
	int channel = 0;
#ifdef WL_DHD_XR_CLIENT
	if (wl_get_drv_status(cfg, CONNECTED, DHD_XR_GET_SLAVE_NDEV(cfg))) {
#else
	if (wl_get_drv_status(cfg, CONNECTED, bcmcfg_to_prmry_ndev(cfg))) {
#endif /* WL_DHD_XR_CLIENT */
		channel = cfg->channel;
...
wl_cfg80211_set_cac(struct bcm_cfg80211 *cfg, int enable)
{
	...
	dhd_pub_t *dhd = (dhd_pub_t *)(cfg->pub);
#ifdef WL_DHD_XR_CLIENT
	dhd = dhd_get_pub(DHD_XR_GET_SLAVE_NDEV(cfg));
...
#ifdef WL_DHD_XR_CLIENT
	ndev = DHD_XR_GET_SLAVE_NDEV(cfg);
	ndev = xr_sta_get_ndev_for_cfg_event(cfg, ndev);
#endif /* WL_DHD_XR_CLIENT */
...
#else
	struct net_device *prim_ndev = bcmcfg_to_prmry_ndev(cfg);
#endif /*WL_DHD_XR*/

#ifdef WL_DHD_XR_MASTER
	if (dhdxr && DHD_GET_XR_ROLE(dhdxr) == XR_SLAVE) {
		return wl_cfg80211_channel_switch_xr(cfg->pub, dhdxr, wiphy, dev, params);
	}
#endif/* WL_DHD_XR_MASTER */
...
#else
		if (DHD_OPMODE_STA_SOFTAP_CONCURR(dhdxr) &&
#endif /* WL_DHD_XR */
			wl_get_drv_status(cfg, CONNECTED,
			prim_ndev) && sta_chan &&
			(*sta_chan != chan)) {
...
#ifdef WL_DHD_XR_MASTER
	if (dhdxr && DHD_GET_XR_ROLE(dhdxr) == XR_SLAVE) {
		return wl_cfg80211_set_cqm_rssi_config_xr(cfg->pub, dhdxr, wiphy, dev, rssi_thold, rssi_hyst);
	}
#endif /* WL_DHD_XR_MASTER */
	...
	struct cfg80211_update_owe_info *owe_info)
{
	...
	if (cfg->xr_sta_mode == XR_STA_MODE_SINGLE
		...
		if ((dev == cfg->wdev->netdev)
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_update_owe_info_xr(cfg->pub, cfg->xr_slave_dhd_pub,
					wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), owe_info);
			}
		}
	}

	if (dhdp && DHD_GET_XR_ROLE(dhdp) == XR_SLAVE) {
		return wl_cfg80211_update_owe_info_xr(cfg->pub, dhdp, wiphy, dev, owe_info);
	}
#endif /* WL_DHD_XR_MASTER */
