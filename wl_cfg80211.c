		...
		if ((dev == cfg->wdev->netdev
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_add_key_xr(cfg->pub, cfg->xr_slave_dhd_pub,
					wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), key_idx, pairwise, mac_addr, params);
			}
		}
	}
		...
		if ((dev == cfg->wdev->netdev)) {
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_del_key_xr(cfg->pub, cfg->xr_slave_dhd_pub,
					wiphy, DHD_XR_GET_SLAVE_NDEV(cfg), key_idx, pairwise, mac_addr);
			}
		}
	}
			...
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				return wl_cfg80211_get_key_xr(cfg->pub,
					cfg->xr_slave_dhd_pub, wiphy, DHD_XR_GET_SLAVE_NDEV(cfg),
						key_idx, pairwise, mac_addr, cookie, callback);
			}
		}
	}
...
s32 wl_xr_slv_init_cfg_event(struct bcm_cfg80211 *cfg)
{
	...
	if (wl_create_event_handler(cfg)) {
		DHD_ERROR(("%s xr slave cfg event handler init failed\n", __func__));
		return BCME_ERROR;
	}
...
wl_cfg80211_ch_switch_notify(struct net_device *dev, uint16 chanspec, struct wiphy *wiphy)
{
	...
	cfg80211_ch_switch_notify(xr_sta_ndev, &chandef, 0);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION (3, 8, 0))
	freq = chandef.chan ? chandef.chan->center_freq : chandef.center_freq1;
	cfg80211_ch_switch_notify(xr_sta_ndev, &chandef);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION (3, 5, 0) && (LINUX_VERSION_CODE <= (3, 7, \
	\
	0)))
	freq = chan_info.freq;
