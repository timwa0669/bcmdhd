		...
		if (type != WLC_E_LINK) {
			...
			if (del_sta) {
				...
				if (role == WLC_E_IF_ROLE_STA || role == WLC_E_IF_ROLE_P2P_CLIENT) {
					...
					/* WAR for SWAUTO-5884 P2p crash issue
					*  Random issue ,During p2p disconnect
					*  which is issued after EAP failure a
					*  delete request is sent but we dont
					*  receive the delete respsonse for 5
					*  seconds during which if any IOVAR is
					*  issued to FW it will timeout leading
					*  to IOVAR Timeout dhd dump.Normally
					*  the delete respsonse is expected in
					*  600us to 1ms.WAR is to wait for 500us
					*  before continuing the disconnect
					*  sequence after flow ring delete request
					*  is issued.
					*/
					if (role == WLC_E_IF_ROLE_P2P_CLIENT)
						osl_delay(500);
				} else {
					dhd_flow_rings_delete_for_peer(dhd_pub, ifindex,
						(char *)&event->addr.octet[0]);
