...
#define PROCFS_DIR_TRACE "dhdx_trace"
...
dhd_dbg_ring_proc_create(dhd_pub_t *dhdp)
{
	...
	if (dbg_verbose_ring) {
		...
		if (dhd_trace_proc_entry == NULL) {
			DHD_ERROR(("Failed to create /proc/"PROCFS_DIR_TRACE" procfs interface\n"));
		} else {
			DHD_ERROR(("Created /proc/"PROCFS_DIR_TRACE" procfs interface\n"));
		}
	} else {
		DHD_ERROR(("dbg_verbose_ring is NULL, /proc/"PROCFS_DIR_TRACE" not created\n"));
	}
#endif /* DEBUGABILITY */
