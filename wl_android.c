...
#ifdef WL_DHD_XR_CLIENT
	skb = CFG80211_VENDOR_EVENT_ALLOC(wiphy, ndev_to_wdev(xr_sta_ndev), len,
		...
		char *command, int total_len)
{
	...
	argc = sscanf(data, "%"BUF_S(BUFSZ)"s %"BUF_S(BUFSZ)"s %"BUF_S(BUFSZ)"s",
					rssi, band, weight);
	...
	char *command, int total_len)
{
	...
	sscanf(data, "%"BUF_S(BUFSZ)"s %"BUF_S(BUFSZ)"s", rssi, band);
...
wl_cfg80211_wbtext_delta_config(struct net_device *ndev, char *data, char *command, int total_len)
{
	...
	argc = sscanf(data, "%"BUF_S(BUFSZ)"s %"BUF_S(BUFSZ)"s", band, delta);
	if (!strcasecmp(band, "a"))
