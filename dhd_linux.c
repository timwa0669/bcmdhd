...
#if defined(CONFIG_ANDROID_VERSION) && ((CONFIG_ANDROID_VERSION <= 11) && \
	(LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 51)))
	...
	if (ret < 0) {
		...
		if ((ndev != bcmcfg_to_prmry_ndev(cfg)) &&
			...
			if (cfg80211_to_wl_iftype(iftype, &wl_iftype, &wl_mode) >= 0) {
				wl_release_vif_macaddr(cfg, (u8 *)ndev->dev_addr, wl_iftype);
			}
		}
#endif	/* WL_CFG80211 */
...
dhd_set_mac_address(struct net_device *dev, void *addr)
{
...
#if (defined(WL_DHD_XR) && defined(CONFIG_ANDROID_VERSION) && (CONFIG_ANDROID_VERSION \
	>= 13)) || (defined(WL_STATIC_IF) && defined(SOFTAP_RAND))
	struct bcm_cfg80211 *cfg = wl_get_cfg(dev);
#endif /* WL_DHD_XR || (WL_STATIC_IF && SOFTAP_RAND) */
	...
	struct net_device *prim_ndev = DHD_XR_GET_SLAVE_NDEV(cfg);
#else
	struct net_device *prim_ndev =  bcmcfg_to_prmry_ndev(cfg);
#endif /* WL_DHD_XR_CLIENT */
#endif /* WL_DHD_XR */
#endif /* SOFTAP_RAND || (CONFIG_ANDROID_VERSION && CONFIG_ANDROID_VERSION >= 13) */
...
#ifdef WL_DHD_XR_MASTER
	if ((net == cfg->wdev->netdev) &&
		((XR_STA_GET_MODE(cfg) == XR_STA_MODE_SINGLE))) {
		/* XR Single sta uses wlan1(slave primary I/F) as proxy
		*   Supplicant sees only wlan0(master primary I/F)
		*   eapol pkts sent from supplicant when the
		*   curent active sta I/F cfg->xr_sta_active_wdev->netdev is wlan1
		*   have to be routed to slave driver data path.
		*/
		xr_sta_ndev = xr_sta_get_ndev_for_cfg_event(cfg, net);
		if (xr_sta_ndev == DHD_XR_GET_SLAVE_NDEV(cfg)) {
		 /* send  l2 pkt to slave */
			DHD_TRACE(("XR STA L2 send to slave\n"));
			skb->dev = xr_sta_ndev;
			if (xr_sta_ndev->netdev_ops->ndo_start_xmit)
				return xr_sta_ndev->netdev_ops->ndo_start_xmit(skb, xr_sta_ndev);
		}
	}
#endif /* WL_DHD_XR_MASTER */
...
#ifdef WL_DHD_XR_CLIENT
			struct bcm_cfg80211 *cfg = wl_get_cfg(skb->dev);
			struct net_device *xr_sta_ndev =
					xr_sta_get_ndev_for_cfg_event(cfg, skb->dev);
#endif /* WL_DHD_XR_CLIENT */
...
#endif /* DHD_4WAYM4_FAIL_DISCONNECT */
#ifdef WL_DHD_XR_CLIENT
		/* XR single sta uses wlan1(slave primary interface)
		* as a proxy for wlan0 wpa_supplicant sees only
		* wlan0(master primary interface) it creates a
		* netlink socket for wlan0 so Eapol packets for
		* wlan1(slave primary interface) has to be routed to
		*  wlan0(master primary interface)
		*/
			skb->dev = xr_sta_ndev;
#endif /* WL_DHD_XR_CLIENT */
		}
...
int dhd_ioctl_process(dhd_pub_t *pub, int ifidx, dhd_ioctl_t *ioc, void *data_buf)
{
	...
	if (pub->busstate == DHD_BUS_DOWN || pub->busstate == DHD_BUS_LOAD) {
		if ((!pub->dongle_trap_occured) && allow_delay_fwdl) {
			...
			if (dhd_download_fw_on_driverload == FALSE && pub->up == FALSE) {
				ret = dhd_bus_devreset(pub, FALSE);
			} else {
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	if (cfg && cfg->xr_sta_mode == XR_STA_MODE_SINGLE && cfg->xr_sta_enabled) {
		if ((net == cfg->wdev->netdev && cfg->xr_sta_active_wdev)) {
			struct net_device *xr_sta_ndev = cfg->xr_sta_active_wdev->netdev;
			dhd_pub_t *xr_sta_pub =
				(dhd_pub_t *) dhd_get_pub(cfg->xr_sta_active_wdev->netdev);
			if (xr_sta_pub && DHD_GET_XR_ROLE(xr_sta_pub) == XR_SLAVE) {
				if (xr_sta_ndev->netdev_ops->ndo_do_ioctl)
					return xr_sta_ndev->netdev_ops->ndo_do_ioctl(xr_sta_ndev,
						ifr, cmd);
			}
		}
	}
...
struct net_info *iter, *next;
	...
	for_each_ndev(cfg, iter, next) {
		GCC_DIAGNOSTIC_POP();
		if (iter->wdev && iter->wdev->iftype == NL80211_IFTYPE_AP && iter->ndev &&
			wl_get_drv_status(cfg, CONNECTED, iter->ndev)) {
			DHD_INFO(("XR AP is UP. skip chip reset. making STA iface down\n"));
			skip_reset = true;
			wl_cfg80211_sta_ifdown(net);
			goto exit;
		}
...
dhd_cleanup_ifp(dhd_pub_t *dhdp, dhd_if_t *ifp)
{
	...
	if (ifp != NULL) {
		...
		if (ifp->idx >= DHD_MAX_IFS) {
			DHD_INFO(("skip! never used idx: %d\n", ifp->idx));
		}
#else
...
{
	return _dhd_remove_if(dhdpub, ifidx, need_rtnl_lock, FALSE);
}
#ifdef WL_STATIC_IF
int dhd_remove_static_if(dhd_pub_t *dhdpub, int ifidx, bool need_rtnl_lock)
{
	return _dhd_remove_if(dhdpub, ifidx, need_rtnl_lock, TRUE);
}
#endif /* WL_STATIC_IF */
...
int
_dhd_remove_if(dhd_pub_t *dhdpub, int ifidx, bool need_rtnl_lock, bool remove_static_if)
{
	dhd_info_t *dhdinfo = (dhd_info_t *)dhdpub->info;
	dhd_if_t *ifp;
#ifdef WL_STATIC_IF
	...
	BCM_REFERENCE(remove_static_if);
	ifp = dhdinfo->iflist[ifidx];
	if (ifp != NULL) {
#ifdef DHD_4WAYM4_FAIL_DISCONNECT
		...
		if (!remove_static_if && ifp->static_if) {
		...
		if (ifp->net != NULL) {
			...
			dhdinfo->iflist[ifidx] = NULL;
#ifdef WL_STATIC_IF
			/* Clearing cfg->static_ndev[x] because ifp->net will be freed below */
			if (remove_static_if) {
				cfg = wl_get_cfg(ifp->net);
				static_if_num = get_iface_num(ifp->net->name, cfg);
				if (static_if_num >= 0) {
					if (ifp->net == cfg->static_ndev[static_if_num]) {
						DHD_INFO(("Clear static_ndev[%d]\n",
							static_if_num));
						cfg->static_ndev[static_if_num] = NULL;
					}
				}
			}
#endif /* WL_STATIC_IF */
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if (wl_cfg80211_dhd_xr_prim_netdev_attach(net, WL_IF_TYPE_STA,
		...
		/* xr slave cfg event handler init */
		if (wl_xr_slv_init_cfg_event(cfg)) {
	...
	dhd_state |= DHD_ATTACH_STATE_HANG_INIT_DONE;
...
#elif defined(CYW43022_CHIP_)
	config_chipid = CYW43022_CHIP_ID;
#elif defined(CYW43023_CHIP_)
	config_chipid = CYW43023_CHIP_ID;
#else
	...
	void *ptr)
{
	...
	switch (event) {
...
#endif /* AOE_IP_ALIAS_SUPPORT */
			if (FW_SUPPORTED(dhd_pub, icmp)) {
				uint32 ipaddr = ifa->ifa_address;
				dhd_iovar(dhd_pub, idx, "icmp_hostip", (char *)&ipaddr,
					sizeof(ipaddr), NULL, 0, TRUE);
			}
			break;
		...
		case NETDEV_DOWN:
			...
			if (FW_SUPPORTED(dhd_pub, icmp)) {
				dhd_iovar(dhd_pub, idx, "icmp_hostip", NULL, 0, NULL, 0, TRUE);
			}
			break;
...
#ifdef WL_STATIC_IF
		/* Cleanup static interfaces */
		for (i = DHD_MAX_IFS; i < DHD_MAX_IFS + DHD_MAX_STATIC_IFS; ++i) {
			if (dhd->iflist[i]) {
				dhd_remove_static_if(&dhd->pub, i, TRUE);
			}
		}
#endif /* WL_STATIC_IF */
		dhd_net_if_unlock_local(dhd);
...
#if defined(OEM_ANDROID)
	if (dhd->dhd_state & DHD_ATTACH_STATE_HANG_INIT_DONE) {
		cancel_work_sync(&dhd->dhd_hang_process_work);
		dhd->dhd_state &= ~DHD_ATTACH_STATE_HANG_INIT_DONE;
	}
#endif /* OEM_ANDROID */
...
dhd_get_random_bytes(uint8 *buf, uint len)
{
	...
	rndlen = get_random_bytes_arch(buf, len);
