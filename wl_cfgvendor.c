...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_set_country(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void *data, int len)
{
	...
	struct net_device *primary_ndev = bcmcfg_to_prmry_ndev(cfg);
        ...
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, ANDR_WIFI_SET_COUNTRY);
        }
...
#if defined(DHD_ANDROID_BRIDGE_SUPPORT)
	struct net_device *ndev;

        ndev= bcmcfg_to_prmry_ndev(cfg);
        ndev->priv_flags &= ~IFF_DONT_BRIDGE;
#if defined(WL_DHD_XR)
        if (DHD_XR_GET_SLAVE_WDEV(cfg)) {
                ndev = (struct net_device *) DHD_XR_GET_SLAVE_NDEV(cfg);
                ndev->priv_flags &= ~IFF_DONT_BRIDGE;
        }
#endif // endif
#endif // endif
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_priv_string_handler(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void  *data, int len)
{
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
	int ret = 0;
	...
	dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, BRCM_VENDOR_SCMD_PRIV_STR);
        }
#endif /* WL_DHD_XR_MASTER */

#if defined(OEM_ANDROID)
#ifdef WL_DHD_XR
	dhdp = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
#else
...
#ifdef WL_DHD_XR_CLIENT
	ndev = wdev->netdev ? wdev->netdev : DHD_XR_GET_SLAVE_NDEV(cfg);
#else
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_set_sae_password(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void  *data, int len)
{
	int err = BCME_OK;
	...
	s32 bssidx;
        ...
        dhd_pub_t *dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, BRCM_VENDOR_SCMD_BCM_PSK);
        }
#endif /* WL_DHD_XR_MASTER */
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_priv_bcm_handler(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void  *data, int len)
{
	const struct nlattr *iter;
	int err = 0;
	...
	u32 cmd_offset;
        ...
        dhd_pub_t *dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, BRCM_VENDOR_SCMD_BCM_STR);
        }
#endif /* WL_DHD_XR_MASTER */
	WL_DBG(("%s: Enter \n", __func__));
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_priv_frameburst(struct wiphy *wiphy,
		struct wireless_dev *wdev, const void *data, int len)
{
	int ret = BCME_OK;
	int val = *(int *)data;
	...
	dhd_pub_t *dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
	if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, BRCM_VENDOR_SCMD_FRAMEBURST);
        }
#endif /* WL_DHD_XR_MASTER */
	if (val == 0xa) {
		ret = wldev_ioctl_get(ndev,
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_priv_mpc(struct wiphy *wiphy,
		struct wireless_dev *wdev, const void *data, int len)
{
	int ret = BCME_OK;
	int val = *(int *)data;
	...
	int err = 0;
        ...
        dhd_pub_t *dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, BRCM_VENDOR_SCMD_MPC);
        }
#endif /* WL_DHD_XR_MASTER */

	if (val == 0xa) {
		ret = wldev_iovar_getint(ndev, "mpc", &get_mpc);
		if (ret) {
			WL_ERR(("Failed : %d\n", ret));
		}
		WL_DBG(("Get MPC : %d\n", get_mpc));
		...
		ret = wldev_iovar_setint(ndev, "mpc", val);
		if (ret < 0) {
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_priv_band(struct wiphy *wiphy,
		struct wireless_dev *wdev, const void *data, int len)
{
	int ret = BCME_OK;
	uint val = *(uint *)data;
	...
	int err = 0;
        ...
        dhd_pub_t *dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, BRCM_VENDOR_SCMD_BAND);
        }
#endif /* WL_DHD_XR_MASTER */
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_priv_muedca_opt_enable(struct wiphy *wiphy,
                struct wireless_dev *wdev, const void *data, int len)
{
	int ret = BCME_OK;
	uint val = *(uint *)data;
	...
	dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, IFX_VENDOR_SCMD_MUEDCA_OPT_ENABLE);
        }
#endif /* WL_DHD_XR_MASTER */
	if (val == 0xa) {
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_priv_ldpc_cap(struct wiphy *wiphy,
		struct wireless_dev *wdev, const void *data, int len)
{
	int ret = BCME_OK;
	int val = *(int *)data;
	...
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
        ...
        dhd_pub_t *dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, IFX_VENDOR_SCMD_LDPC_CAP);
        }
#endif /* WL_DHD_XR_MASTER */
	if (val == 0xa) {
		ret = wldev_iovar_getint(ndev, "ldpc_cap", &get_ldpc_cap);
		if (ret) {
			WL_ERR(("Failed :  %d\n", ret));
		} else {
		...
		ret = wldev_iovar_setint(ndev, "ldpc_cap", val);
		if (ret < 0) {
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_priv_amsdu(struct wiphy *wiphy,
                struct wireless_dev *wdev, const void *data, int len)
{
        int ret = BCME_OK;
        int val = *(int *)data;
        ...
        dhd_pub_t *dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, IFX_VENDOR_SCMD_AMSDU);
        }
#endif /* WL_DHD_XR_MASTER */
        if (val == 0xa) {
                ret = wldev_iovar_getint(ndev, "amsdu", &get_amsdu);
                if (ret) {
                        WL_ERR(("Failed :  %d\n", ret));
                } else {
                        WL_DBG(("Get amsdu : %d\n", get_amsdu));
                ...
                WL_DBG(("Set amsdu %d\n", val));
                ret = wldev_iovar_setint(ndev, "amsdu", val);
                if (ret < 0) {
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_set_pmk(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void *data, int len)
{
	int ret = 0;
	wsec_pmk_t pmk;
	...
	struct wl_security *sec;
        ...
        dhd_pub_t *dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, BRCM_VENDOR_SCMD_SET_PMK);
        }
#endif /* WL_DHD_XR_MASTER */
	nla_for_each_attr(iter, data, len, rem) {
		type = nla_type(iter);
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_get_driver_feature(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void  *data, int len)
{
	int ret = BCME_OK;
...
#endif /* WL_DHD_XR_CLIENT */
#ifdef WL_DHD_XR
	dhd_pub_t *dhd_pub = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
#else
	dhd_pub_t *dhd_pub = cfg->pub;
#endif /* WL_DHD_XR */
	struct sk_buff *skb;
...
#ifdef WL_DHD_XR_MASTER
        if (dhd_pub && DHD_GET_XR_ROLE(dhd_pub) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd_pub, wiphy, wdev, data, len, BRCM_VENDOR_SCMD_GET_FEATURES);
        }
#endif /* WL_DHD_XR_MASTER */
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_set_p2p_rand_mac(struct wiphy *wiphy,
		struct wireless_dev *wdev, const void  *data, int len)
{
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
	int type, err = 0;
        ...
        dhd_pub_t *dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, BRCM_VENDOR_SCMD_SET_MAC);
        }
#endif /* WL_DHD_XR_MASTER */
        if (!data) {
...
static void wl_cfgvendor_acs_offload_work_handler(struct work_struct *work) {
	...
	do {
		...
		ret = nla_put_u32(msg, IFX_VENDOR_ATTR_ACS_PRIMARY_FREQ, report.primary_freq);
		if (ret < 0) {
			WL_ERR(("ACS: Failed to put IFX_VENDOR_ATTR_ACS_PRIMARY_FREQ : %d\n", ret));
			break;
		}
		ret = nla_put_u32(msg, IFX_VENDOR_ATTR_ACS_SECONDARY_FREQ, report.second_freq);
		if (ret < 0) {
			WL_ERR(("ACS: Failed to put IFX_VENDOR_ATTR_ACS_SECONDARY_FREQ : %d\n", ret));
			break;
		}
		ret = nla_put_u8(msg, IFX_VENDOR_ATTR_ACS_VHT_SEG0_CENTER_CHANNEL, report.vht_seg0_center);
		if (ret < 0) {
			WL_ERR(("ACS: Failed to put IFX_VENDOR_ATTR_ACS_VHT_SEG0_CENTER_CHANNEL : %d\n", ret));
			break;
		}
		ret = nla_put_u8(msg, IFX_VENDOR_ATTR_ACS_VHT_SEG1_CENTER_CHANNEL, report.vht_seg1_center);
		if (ret < 0) {
			WL_ERR(("ACS: Failed to put IFX_VENDOR_ATTR_ACS_VHT_SEG1_CENTER_CHANNEL : %d\n", ret));
			break;
		}
		ret = nla_put_u16(msg, IFX_VENDOR_ATTR_ACS_CHWIDTH, report.ch_width);
		if (ret < 0) {
			WL_ERR(("ACS: Failed to put IFX_VENDOR_ATTR_ACS_CHWIDTH : %d\n", ret));
			break;
		}
		ret = nla_put_u8(msg, IFX_VENDOR_ATTR_ACS_HW_MODE, report.hw_mode);
		if (ret < 0) {
			WL_ERR(("ACS: Failed to put IFX_VENDOR_ATTR_ACS_HW_MODE : %d\n", ret));
			break;
		}
...
wl_cfgvendor_acs_offload_init(struct bcm_cfg80211 *cfg, struct wireless_dev *wdev) {
	...
	if (!cfg->acs_offload) {
		...
		if (!cfg->acs_offload) {
			WL_ERR(("ACS: Failed to allocate memory for acs_offload\n"));
			return BCME_NOMEM;
		}
	}
	else {
		int max_try = 10;
		do {
			if (!delayed_work_pending(&((acs_offload_work_t *)cfg->acs_offload)->work))
				break;
			OSL_SLEEP(ACS_OFFLOAD_DELAY);
		} while(max_try--);
		cancel_delayed_work_sync(&((acs_offload_work_t *)cfg->acs_offload)->work);
	}

	acs = (acs_offload_work_t *)cfg->acs_offload;
	acs->selected = 0;
	acs->wdev = wdev;
	return BCME_OK;
}
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_acs_offload(struct wiphy *wiphy,
        struct wireless_dev *wdev, const void  *data, int len)
{
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
	...
	const struct nlattr *iter;
	int rem, type;
	acs_offload_work_t *acs_offload = NULL;
	int ret = BCME_OK;

#if defined(WL_DHD_XR_MASTER)
	dhd_pub_t *dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
	if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
		return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, IFX_VENDOR_SCMD_ACS);
	}
#endif // endif
	if (wl_cfgvendor_acs_offload_init(cfg, wdev) != BCME_OK)
	        return -ENOMEM;

	acs_offload = (acs_offload_work_t *)cfg->acs_offload;

	params = &acs_offload->parameter;

	nla_for_each_attr(iter, data, len, rem) {
		type = nla_type(iter);
		switch(type) {
			case IFX_VENDOR_ATTR_ACS_HW_MODE:
				...
				break;
			case IFX_VENDOR_ATTR_ACS_HT_ENABLED:
				params->ht_enabled = nla_get_u8(iter);
				break;
			case IFX_VENDOR_ATTR_ACS_HT40_ENABLED:
				params->ht40_enabled = nla_get_u8(iter);
				break;
			case IFX_VENDOR_ATTR_ACS_VHT_ENABLED:
				params->vht_enabled = nla_get_u8(iter);
				break;
			case IFX_VENDOR_ATTR_ACS_CHWIDTH:
				params->ch_width = nla_get_u16(iter);
				break;
			case IFX_VENDOR_ATTR_ACS_FREQ_LIST:
				if (nla_len(iter)) {
...
int
wl_cfgvendor_acs_offload_post(struct wiphy *wiphy)
{
	struct bcm_cfg80211 *cfg;
	struct net_device *ndev;
	acs_offload_work_t *acs_offload = NULL;
	...
	list_cnt = 0;
	for (i=0;i<params->chan_cnt;++i) {
		if (band == WL_CHANNEL_BAND(params->channels[i]) )
			list->element[list_cnt++] = wl_ch_host_to_driver(params->channels[i]);
	}
	list->count = list_cnt;
	...
	buflen = sizeof(wl_uint32_list_t) + sizeof(uint32) * (list->count);
...
failed:
	...
	schedule_delayed_work(&acs_offload->work,
		     ...
		     const void  *data, int len)
{
	...
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
#ifdef WL_DHD_XR_CLIENT
        struct net_device *pri_ndev = DHD_XR_GET_SLAVE_NDEV(cfg);
#else
	...
	wl_twt_param_t twt_param = {
        ...
        dhd_pub_t *dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, IFX_VENDOR_SCMD_TWT);
        }
#endif /* WL_DHD_XR_MASTER */
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_oce_enable(struct wiphy *wiphy,
		struct wireless_dev *wdev, const void *data, int len)
{
	int ret = BCME_OK;
	...
	dhd_pub_t *dhd = NULL;
#endif /* WL_DHD_XR_MASTER */
	/* 256 bytes length buffer is not sufficient for this vendor command.
	 * Hence buffer size is increasted to 512 bytes */
	uint8 oce_xtlv[WLC_IOCTL_SMLEN + WLC_IOCTL_SMLEN] = {0};
	...
	dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, IFX_VENDOR_SCMD_OCE_ENABLE);
        }
#endif /* WL_DHD_XR_MASTER */
	if (val == 0xa) {
		...
		buflen = buflen_start = WLC_IOCTL_SMLEN + WLC_IOCTL_SMLEN - sizeof(bcm_iov_buf_t);
...
static
#endif /* WL_DHD_XR */
int wl_cfgvendor_randmac(struct wiphy *wiphy,
              struct wireless_dev *wdev, const void *data, int len)
{
        int ret = BCME_OK;
	...
	dhd = (dhd_pub_t *)dhd_get_pub(wdev->netdev);
        if (dhd && DHD_GET_XR_ROLE(dhd) == XR_SLAVE) {
                return wl_cfgvendor_cmd_xr(cfg->pub, dhd, wiphy, wdev, data, len, IFX_VENDOR_SCMD_RANDMAC);
        }
#endif /* WL_DHD_XR_MASTER */
        if (val == 0x1) {
...
static int wl_cfgvendor_wnm(struct wiphy *wiphy,
              struct wireless_dev *wdev, const void *data, int len)
{
	const struct nlattr *attr_iter, *wnm_param_iter;
	u8 param[64] = {0}, get_info = 0;
	u16 buf_len = 0, wnm_id = 0;
	int err = BCME_ERROR, ret = BCME_OK;
	int tmp, attr_type = 0, wnm_param = 0, get_period = 0;
	s32 bssidx = 0;
	struct net_device *dev;
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
	dev = wdev_to_ndev(wdev);
	...
	nla_for_each_attr(attr_iter, data, len, tmp) {
		...
		case IFX_VENDOR_ATTR_WNM_CMD:
			if (nla_len(attr_iter) != sizeof(uint8)) {
				WL_ERR(("nla_len not matched, attr_type = %d.\n", attr_type));
				ret = -EINVAL;
				goto exit;
			}
		...
		case IFX_VENDOR_ATTR_WNM_PARAMS:
			nla_for_each_nested(wnm_param_iter, attr_iter, tmp) {
				...
				switch (wnm_param) {
				case IFX_VENDOR_ATTR_WNM_MAXIDLE_PARAM_GET_INFO:
				{
					if (nla_len(wnm_param_iter) != sizeof(uint8)) {
						WL_ERR(("nla_len not matched, wnm_param = %d.\n", wnm_param));
						ret = -EINVAL;
						goto exit;
					}
					...
					break;
				case IFX_VENDOR_ATTR_WNM_MAXIDLE_PARAM_IDLE_PERIOD:
				{
					int period;
					...
					break;
				case IFX_VENDOR_ATTR_WNM_MAXIDLE_PARAM_PROTECTION_OPT:
				{
					int option;
					...
					break;
				default:
					WL_ERR(("unknown wnm param attr:%d\n", wnm_param));
					return -EINVAL;
				}
			}
			break;
		default:
			WL_ERR(("Unknown wnm attribute %d, skipping\n",
				  attr_type));
			return -EINVAL;
		}
	}
	...
	switch (wnm_id) {
	case IFX_WNM_CONFIG_CMD_IOV_WNM_MAXIDLE:
	{
		if (get_info) {
			ret = wldev_iovar_getint(bcmcfg_to_prmry_ndev(cfg), "wnm_maxidle", &get_period);
			if (!ret) {
				err = wl_cfgvendor_send_cmd_reply(wiphy, &get_period, sizeof(int));
				if (unlikely(err)) {
					WL_ERR(("Vendor Command reply failed ret:%d \n", err));
					return err;
				}
			...
			err = wldev_iovar_setbuf_bsscfg(dev, "wnm_maxidle", (void *)param, buf_len,
				cfg->ioctl_buf, WLC_IOCTL_SMLEN, bssidx, &cfg->ioctl_buf_sync);
			if (unlikely(err)) {
				WL_ERR(("Failed to set wnm_maxidle enable, error=:%d\n", err));
				return err;
			}
		}
	}
	...
	default:
		WL_ERR(("unsupport wnm cmd:%d\n", wnm_id));
		return -EINVAL;
	}

	if (ret)
		WL_ERR(("wnm %s error:%d\n", get_info?"get":"set", ret));
...
#endif /* WL11AX */
#ifdef WL_SUPPORT_ACS_OFFLOAD
const struct nla_policy acs_offload_attr_policy[IFX_VENDOR_ATTR_ACS_LAST] = {
        [IFX_VENDOR_ATTR_ACS_CHANNEL_INVALID] = { .type = NLA_U8 },
        [IFX_VENDOR_ATTR_ACS_PRIMARY_FREQ] = { .type = NLA_U32 },
        [IFX_VENDOR_ATTR_ACS_SECONDARY_FREQ] = { .type = NLA_U32 },
        [IFX_VENDOR_ATTR_ACS_VHT_SEG0_CENTER_CHANNEL] = { .type = NLA_U8 },
        [IFX_VENDOR_ATTR_ACS_VHT_SEG1_CENTER_CHANNEL] = { .type = NLA_U8 },
        [IFX_VENDOR_ATTR_ACS_HW_MODE] = { .type = NLA_U8 },
        [IFX_VENDOR_ATTR_ACS_HT_ENABLED] = { .type = NLA_U8 },
        [IFX_VENDOR_ATTR_ACS_HT40_ENABLED] = { .type = NLA_U8 },
        [IFX_VENDOR_ATTR_ACS_VHT_ENABLED] = { .type = NLA_U8 },
        [IFX_VENDOR_ATTR_ACS_CHWIDTH] = { .type = NLA_U16 },
        [IFX_VENDOR_ATTR_ACS_CH_LIST] = { .type = NLA_BINARY },
        [IFX_VENDOR_ATTR_ACS_FREQ_LIST] = { .type = NLA_BINARY },
};
#endif // endif
...
/* Registration of google twt teardown request vendor command handler */
        ...
        },
	{
        ...
        },
		{
			{
				.vendor_id = OUI_IFX,
				.subcmd = IFX_VENDOR_SCMD_WNM
			},
				.flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
				.doit = wl_cfgvendor_wnm,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 3, 0))
				.policy = ifx_vendor_attr_wnm_policy,
...
static const struct  nl80211_vendor_cmd_info wl_vendor_events [] = {
		...
		{ OUI_IFX, IFX_VENDOR_EVENT_XR_CONNECTED},
		{ OUI_IFX, IFX_VENDOR_EVENT_XR_STA_MAC_CHANGE},
};
...
#ifdef WL_SUPPORT_ACS_OFFLOAD
	wl_cfgvendor_acs_offload_deinit((acs_offload_work_t *)cfg->acs_offload);
#endif // endif
