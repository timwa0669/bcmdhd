...
#else
#define ETH_HW_ADDR_SET(dev, addr) memcpy_s((void *)dev->dev_addr, \
									 ETH_ALEN, addr, ETH_ALEN)
...
#define PCI_FREE_CONSISTENT(pdev, size, va, handle) \
		dma_free_coherent(&((struct pci_dev*) (pdev))->dev, size, va, handle)
#define PCI_MAP_SINGLE(pdev, va, size, dir) \
		dma_map_single(&((struct pci_dev*) (pdev))->dev, va, size, dir)
#define PCI_DMA_MAPPING_ERROR(pdev, addr) \
		dma_mapping_error(&((struct pci_dev*) (pdev))->dev, addr)
#define PCI_UNMAP_SINGLE(pdev, addr, size, dir) \
		dma_unmap_single(&((struct pci_dev*) (pdev))->dev, addr, size, dir)
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 3, 42))
#define PCI_FREE_CONSISTENT(pdev, size, va, handle)	pci_free_consistent(pdev, size, va, handle)
#define PCI_MAP_SINGLE(pdev, va, size, dir)	pci_map_single(pdev, va, size, dir)
#define PCI_DMA_MAPPING_ERROR(pdev, addr)	pci_dma_mapping_error(pdev, addr)
#define PCI_UNMAP_SINGLE(pdev, addr, size, dir)	pci_unmap_single(pdev, addr, size, dir)
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(2, 3, 42))
...
#else
#define COMPLETE_AND_EXIT(comp, code)	complete_and_exit(comp, code)
#endif /* KERNEL_VERSION >= (5, 17, 0) */
...
#define RANDOM_BYTES	get_randon_bytes
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0))
