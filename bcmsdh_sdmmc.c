...
#if defined(PLATFORM_IMX) && defined(FCIPA_CHIP)
/* In IMX8 FCIPA(60pin) chips works with 100Mhz and M2IPA chips works with default clock (200Mhz)
 * No need of setting clock for M2IPA chips
 */
#define SD_CLOCK_UHS_IMX8 100000000  /* 100Mhz */
#endif /* PLATFORM_IMX && FCIPA_CHIP */
		...
		if (func == 0) {
			 ...
			 */
				...
				if (sd->func[func]) {
					sdio_claim_host(sd->func[func]);
#ifdef DHD_43022
					/* sdio_f0_writeb only allows to read/write between 0xF0 to
					 * 0xFF address. So use sdio_writeb To access other address
					 * (Ex:SDIOD_CCCR_BICTRL-0x07).
					 */
					sdio_writeb(sd->func[func],
						*byte, regaddr, &err_ret);
#else
