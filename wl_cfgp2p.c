...
{
	if (cfg->p2p) {
		CFGP2P_INFO(("struct p2p_info is already allocated\n"));
		return BCME_OK;
	}
...
#ifdef WL_6E
		err = wldev_iovar_getint(dev, "chanspec", &chnsp);
		if (unlikely(err)) {
			CFGP2P_ERR((" Chanspec failed %d\n",err));
			return err;
		}
...
#if defined(WL_NEWCFG_PRIVCMD_SUPPORT)
	if (cfg->p2p_net)
		ETH_HW_ADDR_SET(cfg->p2p_net, wl_to_p2p_bss_macaddr(cfg, P2PAPI_BSSCFG_DEVICE));
#endif /* WL_NEWCFG_PRIVCMD_SUPPORT */
